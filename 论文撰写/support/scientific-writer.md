...
**Required Environment Variable:** `33buSbpZKy1rjCdOoWSIG0SGXi720UqiA5jpwUe`

如何调用parallel
curl -X POST https://api.parallel.ai/v1beta/search \
  -H 'Content-Type: application/json' \
  -H 'x-api-key: T33buSbpZKy1rjCdOoWSIG0SGXi720UqiA5jpwUe' \
  -d '{
  "mode": "one-shot",
  "search_queries": null,
  "max_results": 10,
  "objective": "Energy and Exergy Analysis and Optimization of Combined Heat and Power Systems. Comparison of Various Systems"
}'  
...
2. **Execute with Continuous Updates**
...
5. **AUTOMATIC PDF Formatting Review (Required After Each Compilation)**
  **CRITICAL: This step is MANDATORY after any PDF is generated.**
  ...
  10. **TENTH**: Re-run image conversion and inspection (iterate until clean)
    ...
    Fs cannot be properly interpreted by reading the binary file directly. You MUST convert
    e PDF to images and then read the images for visual inspection.
    MANDATORY Image Conversion Workflow (No Exceptions):**
  11. **FIRST**: Print: `[HH:MM:SS] PDF REVIEW: Converting PDF to images for visual inspection`
  12. **SECOND**: Create review directory if not exists: `mkdir -p review/`
  13. **THIRD**: Convert ALL PDF pages to images using Python:
    ```bash
    ...
    ```
  14. **FOURTH**: Print: `[HH:MM:SS] PDF REVIEW: Converted [N] pages to images in review/ directory`
  15. **FIFTH**: Count number of page images created
  16. **SIXTH**: Read and inspect EACH page image file sequentially (page-1.jpg, page-2.jpg, etc.):
  ...
  17. **SEVENTH**: After all page images reviewed:
    - Print: `[HH:MM:SS] PDF REVIEW: Completed image-based review - [N] total issues found`
    - List specific issues with page numbers
  18. **EIGHTH**: If issues found, apply fixes to source (.tex), recompile, and re-review
  ...
  19. **Text Overlaps**: Text overlapping with figures, tables, equations, or margins
  20. **Phantom Spaces**: Excessive whitespace, awkward gaps between sections, orphaned lines
  21. **Figure Placement**: Figures appearing far from references, overlapping text
  22. **Table Issues**: Tables extending beyond margins, poor alignment, caption spacing
  23. **Section Breaks**: Inconsistent spacing between sections, awkward page breaks
  24. **Margins**: Text/figures bleeding into margins or inconsistent margins
  25. **Page Breaks**: Sections/subsections starting at bottom of page, widows/orphans
  26. **Caption Spacing**: Too much/little space around figure/table captions
  27. **Bibliography**: Reference list formatting, hanging indents, spacing
  28. **Equation Spacing**: Equations overlapping text or poorly positioned
    Review Process:**
     **Initial Review:**
     ...
     **Cleanup Review Images (MANDATORY after review cycle completes):**
...
6. **DOI**: Always include when available, verify resolves at [https://doi.org/](https://doi.org/)
...
1. **Figure Generation Using Scientific Schematic Skill**
