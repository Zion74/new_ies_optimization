...
### 4.4 烟厂 test 模式轻量双层优化验证

为增强中期验收说服力，额外对烟厂场景运行了“外层容量候选搜索 + 内层 24h 调度求解”的轻量双层流程。该流程不是最终正式容量优化器，但已经验证了通用模块库能够按动态容量变量生成候选方案，并把候选容量传入通用调度模型进行可行性判断和目标值计算。
...
- 求解器终止状态：`optimal`
- 调度目标值：`78835.84819047617`
- 结果目录：`DesignResults/tobacco_factory_bilevel_test/`

本次验证同时修复了一个重要边界问题：当外层搜索产生不可行容量时，程序现在会把内层调度 `infeasible` 的候选方案标记为不可行，并优先选择可行调度方案作为 `best_solution`，避免把“投资成本为 0 但不可调度”的候选误认为最优结果。
