...
3. `系统设计/中期后开发重点与路线图.md`：中期后继续开发通用后端、双层优化、场景接入和合作组接口时先看。
4. `课题组场景整理/烟厂场景接入评估与数据清洗记录.md`：第三场景数据来源、清洗、容量边界和 Level 3 求解记录。
5. `ies_design/README.md`：开发者入口，查看代码结构、CLI 命令和测试脚本。
6. `统一建模/源荷储转设备库统一建模接口规范.md`：理解设备库抽象、EMS 对接和源荷储转统一建模边界。
...
| `系统设计/中期后开发重点与路线图.md` | 沉淀中期后通用后端、双层优化、场景接入、烟厂深化、15 场景扩展和合作课题组对接路线。 |
