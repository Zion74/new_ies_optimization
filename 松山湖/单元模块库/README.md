...
| `烟厂场景接入评估与数据清洗记录.md` | 评估卷烟厂作为第三真实结构场景的接入可行性，记录 Excel 清洗、原始数据核查、结题材料容量约束和 future_generic 接入路径。 | 烟厂场景、第三场景、数据清洗、future_generic |
| `课题组场景整理模板_烟厂_清洗版.xlsx` | 烟厂场景 Excel 清洗副本，修复 04 表负荷重复/错位问题，扩展 05 表资源曲线，并补充热泵与储热容量约束。 | Excel模板、烟厂场景、清洗版 |
| `ies_design/scenarios/tobacco_factory/` | 烟厂第三真实结构场景包，包含 `scenario.yaml`、典型日负荷、资源曲线和数据缺口表，可导出 `future_generic` 组件计划。 | 第三场景、future_generic、场景包 |
