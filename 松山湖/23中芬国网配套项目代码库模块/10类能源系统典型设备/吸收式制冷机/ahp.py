...
solver = "gurobi"  # 选择求解器
debug = True  # 是否输出LP文件
number_of_time_steps = 24  # 设置调度时间跨度
solver_verbose = True  # 是否输出求解器信息

# 初始化日志
logger.define_logging(
    logfile="bin_jiang_base.log",
...
date_time_index = pd.date_range("1/10/1980", periods=number_of_time_steps, freq="H")
energy_system = solph.EnergySystem(timeindex=date_time_index)

# 读取数据文件
filename = os.path.join(os.getcwd(), "bin_jiang_base.csv")
data = pd.read_csv(filename)


# 设置AHP机组参数
ahp = solph.Transformer(
    label='AHP',
    inputs={heat_bus: solph.Flow(), ele_bus: solph.Flow()},
    outputs={cool_bus: solph.Flow(nominal_value=212)},
