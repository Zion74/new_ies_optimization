"""Gate A compiler and BESS-only monitored executor for formal E0-D-49."""
...
from tes_bess_boundary.e0d48_monitored_executor import (
    _guide_mapping,
    _parse_junit,
...
EXECUTION_SCHEMA_ID = "tes_bess_boundary.e0d49_stage_execution.v1"
ARCHITECTURE_SCHEMA_ID = "tes_bess_boundary.e0d49_architecture_manifest.v1"
FORMAL_SCHEMA_ID = "tes_bess_boundary.e0d49_formal_manifest.v1"
GATE_A_SCHEMA_ID = "tes_bess_boundary.e0d49_gate_a_manifest.v1"

FORMAL_ARCHITECTURE = Architecture.BESS
FORMAL_ARCHITECTURE_ORDER = (FORMAL_ARCHITECTURE,)
...
def build_stage_command(
    *,
    ...
    """Build the only two registered child commands for BESS."""
    ...
    raise ValueError(f"unknown D49 stage: {stage}")
...
def run_monitored_stage(
    *,
    ...
    """Run one clean child under the frozen process-tree resource gate."""

    if not math.isfinite(hard_wall_seconds) or hard_wall_seconds <= 0.0:
        raise ValueError("D49 stage hard wall must be finite and positive")
    paths = _stage_paths(output_dir, stage)
    ...
    for path in (
        ...
        *artifacts,
        ...
        if path.exists():
            raise FileExistsError(f"D49 refuses to overwrite {path}")
    available_before = _available_memory_gib()
    if available_before is None:
        raise RuntimeError("D49 formal execution requires Linux /proc")
    if available_before < HOST_MEMORY_RESERVE_GIB:
        raise RuntimeError("D49 host memory is below the frozen reserve")
...
def _load_completed_result(output_dir: Path, stage: str) -> dict[str, Any] | None:
    ...
    if execution.get("result_sha256") != _sha256(paths["result"]):
        raise ValueError(f"D49 BESS {stage} result hash mismatch")
...
def run_bess_method_gate(
    *,
    ...
    """Run exactly one candidate and, only after exact lift, one repair."""
    ...
    if (
        ...
        and candidate.get("status") == "candidate_incumbent_captured_and_exactly_lifted"
    ):
        remaining = hard_wall_seconds - (perf_counter() - started)
        if remaining > 0.0:
    ...
    elif candidate.get("status") == "engineering_mip_infeasible_under_projection":
        status = "engineering_mip_infeasible_under_projection"
    elif candidate.get("status") == "candidate_found_but_exact_lift_failed":
        status = "candidate_found_but_exact_lift_failed"
    elif candidate.get("status") != "candidate_incumbent_captured_and_exactly_lifted":
        status = "no_primal_status_closure"
    else:
...
def compile_gate_a_manifest(
    *,
    ...
    """Compile all result-before Gate A evidence without formal optimization."""

    if output_dir.exists():
        raise FileExistsError(f"D49 Gate A output already exists: {output_dir}")
    if re.fullmatch(r"[0-9a-f]{40}", git_commit) is None:
        raise ValueError("D49 Gate A requires a full lowercase Git commit")
    started = perf_counter()
    ...
    projected_sizes: dict[str, Any] = {}
    for architecture in (Architecture.BESS, Architecture.TES, Architecture.HYBRID):
        path = build_dir / f"gate_a_build_{architecture.value}.json"
        payload = json.loads(path.read_text(encoding="utf-8"))
        if payload.get("schema_id") != BUILD_SCHEMA_ID:
            raise ValueError(f"D49 Gate A {architecture.value} build schema mismatch")
        if payload.get("status") != "gate_a_build_passed":
            raise ValueError(f"D49 Gate A {architecture.value} build failed")
        if payload.get("solver_invoked") is not False:
            raise ValueError(f"D49 Gate A {architecture.value} invoked a solver")
        if payload.get("formal_optimization_invoked") is not False:
            raise ValueError(f"D49 Gate A {architecture.value} invoked formal optimize")
        if payload.get("audit", {}).get("passed") is not True:
            raise ValueError(f"D49 Gate A {architecture.value} audit failed")
        preparation = payload["physics_first_preparation_audit"]
        if preparation["fuel_projection_dependency_audit"]["passed"] is not True:
            raise ValueError(f"D49 Gate A {architecture.value} dependency proof failed")
        if payload["static_fuel_lift_spec_audit"]["passed"] is not True:
            raise ValueError(
                f"D49 Gate A {architecture.value} static lift proof failed"
            )
        build_hashes[architecture.value] = _sha256(path)
        partition_audits[architecture.value] = preparation["binary_partition_audit"]
        projected_sizes[architecture.value] = payload["projected_model_size_audit"]
    tests = {
        "d49_targeted": _parse_junit(targeted_junit_path, no_skips=True),
        "d40_d49_compatibility": _parse_junit(compatibility_junit_path, no_skips=True),
        "full_package": _parse_junit(full_junit_path, no_skips=True),
    }
    if not all(item["passed"] for item in tests.values()):
        raise ValueError("D49 Gate A test evidence failed")
    quality = {
        "ruff": {
            "sentinel_present": "D49_RUFF_PASSED"
            in ruff_log_path.read_text(encoding="utf-8"),
            "file_sha256": _sha256(ruff_log_path),
        },
        "pycompile": {
            "sentinel_present": "D49_PYCOMPILE_PASSED"
    ...
    if not all(item["sentinel_present"] for item in quality.values()):
        raise ValueError("D49 Gate A quality sentinel is missing")
    ...
    payload = {
        ...
        "binary_partition_audit": partition_audits,
        "projected_model_size_audit": projected_sizes,
        "test_evidence": tests,
        ...
        "provenance": {
            ...
            "test_sha256": {
                ...
                for name in (
                    "test_e0d49_physics_first_fuel_projection_primal_recovery.py",
                    "test_e0d49_monitored_executor.py",
                )
            },
        },
        "audit": {"passed": True},
...
def _validate_gate_a(manifest_path: Path, execution_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_id") != GATE_A_SCHEMA_ID:
        raise ValueError("D49 Gate A schema mismatch")
    if manifest.get("status") != "gate_a_passed":
        raise ValueError("D49 formal run requires a passed Gate A")
    if manifest.get("formal_optimization_invoked") is not False:
        raise ValueError("D49 Gate A unexpectedly invoked formal optimization")
    if manifest.get("formal_architecture_order") != [FORMAL_ARCHITECTURE.value]:
        raise ValueError("D49 Gate A did not authorize BESS-only formal execution")
    if manifest.get("tes_or_hybrid_formal_run_permitted") is not False:
        raise ValueError("D49 Gate A improperly authorized TES/Hybrid")
    if manifest.get("formal_run_permitted") is not True:
        raise ValueError("D49 Gate A did not permit the formal run")
    if manifest.get("audit", {}).get("passed") is not True:
        raise ValueError("D49 Gate A audit is not passed")
    if execution.get("status") != "complete":
        raise ValueError("D49 Gate A execution is incomplete")
    if execution.get("manifest_sha256") != manifest_hash:
        raise ValueError("D49 Gate A execution hash mismatch")
    if manifest.get("provenance", {}).get("code_sha256") != _code_hashes():
        raise ValueError("D49 formal source differs from Gate A")
    return {
...
def run_formal_bess_gate(
    *,
    ...
    """Run the only pre-registered D49 formal method gate: BESS once."""

    if output_dir.exists():
        raise FileExistsError(f"D49 formal output already exists: {output_dir}")
    ...
    if available is None or available < HOST_MEMORY_RESERVE_GIB:
        raise RuntimeError("D49 formal host does not satisfy the memory reserve")
    output_dir.mkdir(parents=True)
    started = perf_counter()
    bess = run_bess_method_gate(
        output_dir=output_dir,
        guide_path=guide_paths[FORMAL_ARCHITECTURE],
        service_path=service_path,
        d40_gate_a_manifest_path=d40_gate_a_manifest_path,
        d41_gate_a_manifest_path=d41_gate_a_manifest_path,
        heat_path=heat_path,
        vre_path=vre_path,
        price_basis_path=price_basis_path,
        hard_wall_seconds=ARCHITECTURE_HARD_WALL_SECONDS,
    )
    artifact_hashes = {
...
def main() -> None:
    ...
    if args.command == "formal-bess":
        ...
        return
    raise AssertionError(f"unhandled D49 executor command: {args.command}")
