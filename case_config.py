...
SONGSHAN_LAKE_CASE = {
    ...
    # ---- 决策变量上界 [ppv, pwt, pgt, php, pec, pac, pes, phs, pcs] ----
    # 松山湖规模较小，冷负荷主导（实际同时峰值约3460kW，图5延时曲线）
    # pwt=0: 东莞城区风资源差，不配风电
    # pec需要足够大以覆盖冷负荷峰值（EC+AC+CS需≥3460kW）
    ...
    # ---- 设备效率参数（基于CAT官方手册 + PDF表4反算 + 行业经验，2026-04-16核查确认）----
    "gt_eta_e": 0.423,   # CAT G3512E 官方 Maximum Electrical Efficiency (42.3%)
    "gt_eta_h": 0.42,    # 综合效率≥80% - 42.3%发电 ≈ 40-45%，取中值42%
    "ac_cop": 0.70,      # 烟气热水型溴化锂机组（单效，驱动热源~90°C，行业值0.70-0.75）
    "ac_heat_ratio": 0.983,
    "ac_ele_ratio": 0.017,
    "ehp_cop": 4.0,      # 电热泵COP（亚热带气候）
    "ec_cop": 5.5,       # 1500kW级水冷离心/螺杆机组（行业值5.0-6.5，取折中5.5）
    "es_charge_eff": 0.95,
