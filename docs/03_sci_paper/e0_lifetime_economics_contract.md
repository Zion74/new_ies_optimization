...
E0-D-18 当前更新：实际 E0-C 年度解已经接入该内核；24 h 冬季典型日燃料范围阈值可精确跨平台复现，336 h 窗口以主 gap 0.004800 形成 EAC 上下界。TES 正式 portfolio 与 VOM/碳/电力结算仍未闭合，因此两窗口结果仍只允许探索使用，不改变正式 TAC 门槛。当前本地/远端完整回归均为 288 项通过。
