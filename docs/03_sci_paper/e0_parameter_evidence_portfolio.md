...
上述剩余门槛未全部满足前，E0 保持“完整 fixed-capacity BESS 生命周期账本已闭合，但全部 TES 正式成本与系统级 TAC 未闭合”，E1 与批量边界扫描不启动。E0-D-16/17 只增加探索性全系统 TES EAC 上限、实际结果适配和 24 h 筛查，不把阈值写回参数表。当前完整回归为本地 `284 passed in 76.16s`、OpenBayes `284 passed in 21.37s`；24 h canonical 跨平台一致，两周性能门未通过。
