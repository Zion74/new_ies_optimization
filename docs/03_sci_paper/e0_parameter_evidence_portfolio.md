...
上述剩余门槛未全部满足前，E0 保持“完整 fixed-capacity BESS 生命周期账本已闭合，但全部 TES 正式成本与系统级 TAC 未闭合”，E1 与批量边界扫描不启动。E0-D-16 只增加探索性全系统 TES EAC 上限与物理价值差值，不把阈值写回参数表。本地完整回归为 `279 passed in 31.57s`（关闭 pytest cache）；OpenBayes 最近仍为 `258 passed in 21.36s`，本轮尚未同步。
