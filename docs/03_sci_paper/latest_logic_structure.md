...
- E0-D-38-R1 随后在任何修订状态结果产生前一次性冻结对称 `H*=G*=0.70`：热量尺度 `1.5962316499799991`、峰值 `724.034 MWth`，相对静态上限保留 `42.043 MWth`。R1 不改代表周、PCC、服务、成本、gap 或验收规则，也不允许再次调参；只有 R1 三状态全部通过才可继续 E2/E3/E4；
- 原始证据审计确认杨凌表内没有供回水温度、抽汽温压或可直接识别三罐逐时损失/泵耗的设备参数。MT 与损失/泵耗仍为作者敏感性，公开成本不能闭合项目账户。D34–D37 已关闭实现、材料性、代表周和分块边界缺口；原 D38 高热状态已失败，R1 已结果前冻结但未求解。TES/系统正式 TAC、有效时间聚合门、正式 E5 与 336 h 宽外界继续阻断正式结论；下一步只可按 R1 执行，不得启动 E2 正式经济前沿或批量赢家扫描。
