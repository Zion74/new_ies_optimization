...
- E0-D-23 建立两架构联合 MILP；E0-D-26 将年度成本/弃电 cap 无量纲化，使用 `1e-9` 严格可行性容差，分开 D19 条件整数面与全部整数模式，并要求全局 incumbent 支配已知证人。E0-D-27 又用不含符号二元的固定支持方向产生可行 L1，并用正负差值分解取代 D23 的 `2M` 绝对值上界。24 h 全局严格包络修正为 `26,010.171143–26,010.174929 MWh/a`，D22 选择值仍几乎位于最大端；336 h 最小严格区间沿用 `0–15,594.993900 MWh/a`，最大严格区间收紧为 `36,382.462799–1,081,649.139331 MWh/a`。336 h 固定方向 dual `42,290.912450 MWh/a` 不是全局上界；正负分解有限 dual 才是全局外界，但运行达 1800 s 时限且 gap 仍宽，因此 336 h 尚未闭合；
