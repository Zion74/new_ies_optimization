...
- E0-D-23 建立两架构联合 MILP；E0-D-26 针对 D23 的约束尺度与默认容差问题重新发证。D26 将年度成本/弃电 cap 无量纲化，使用 `1e-9` 严格可行性容差，分开 D19 条件整数面与全部整数模式，并要求全局 incumbent 支配已知条件面证人。24 h 全局严格包络修正为 `26,010.171143–26,010.174918 MWh/a`，D22 选择值仍几乎位于最大端；336 h 最小严格区间收紧为 `0–15,594.993900 MWh/a`，最大严格区间为 `36,382.462799–1,362,149.106858 MWh/a`。336 h 条件面最小值精确闭合，但条件面最大值只有可行 incumbent、无有限 dual；因此全局包络仍未闭合，任何 `optimal` 标签都必须同时通过有限界、证人支配和 PCC 重算审计；
