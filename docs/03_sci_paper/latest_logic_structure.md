...
- E0-D-39 已在任何新数据或求解结果产生前冻结一次性服务感知修订：保留 D36 第 `4/5/8/29/39/48` 周，并按 D38 baseline 周级弃电低估降序加入第 `49/16` 周；全部 52 周沿用 D36 的 672 维特征距离重分配，年尾 72 h 不变。先验 gate 要求代表期与真实全年对 no-storage 10% 服务的可行性分类一致，且自然最小弃电率误差不超过 1 个百分点；失败不允许继续加周或放宽阈值；
- 原始证据审计确认杨凌表内没有供回水温度、抽汽温压或可直接识别三罐逐时损失/泵耗的设备参数。MT 与损失/泵耗仍为作者敏感性，公开成本不能闭合项目账户。D34–D37 已关闭实现、材料性、代表周和分块边界缺口；原 D38 高热状态与 R1 baseline 时间聚合门均已失败，D39 仅完成结果前冻结、尚无数据或结果。当前双端完整回归为 `445 passed`。TES/系统正式 TAC、有效时间聚合门、正式 E5 与 336 h 宽外界继续阻断正式结论；下一步只可实现并执行 D39 Gate A/B，不得启动 E2 正式经济前沿或批量赢家扫描。
