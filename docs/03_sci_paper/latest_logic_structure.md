...
- E0-D-43 已在任何实现或正式 dual 读取前冻结只读离线证书恢复合同。两个 D42 solution 归档均含完整 `439,018` 维 row dual，`dual_valid=true` 且绑定同一 presolve LP；D43 固定两个 clean child 并行、每个 `1,800 s` 硬墙钟和 Decimal 最大合法下界选择，不调用优化器。当前尚无 D43 数值；
- 原始证据审计确认杨凌表内仍缺项目级 TES 参数和完整账户。D41/D42 的 BESS 下界不是原 MILP 可行解、容量方案或项目 TAC。当前仍没有三架构可比的容量、成本、gap 或赢家。正式 E2–E4、Hybrid、全年上界修复与技术排序继续阻断；这不证明 TES 不可行或 BESS 更优。下一步只实现、回归并提交 D43 只读编排器，不能在 D42 名下延时或改规则。
