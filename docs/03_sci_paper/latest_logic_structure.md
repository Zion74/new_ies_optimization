...
- 原始证据审计确认杨凌表内仍缺项目级 TES 参数和完整账户。BESS/TES 当前拥有来源不同的合法下界，但都不是原 MILP 可行解、容量方案或项目 TAC，不能以 `1,144,950,604.8368804` 对 `254,860,566.6193158889 CNY` 直接排序。D45 未形成 Hybrid 数值下界且不得重跑，`d46_feasible_upper_bound_contract_permitted=false`；下一步只能先另立缩放、对偶修复、R1 分支或严格分解型 Hybrid 下界恢复合同，E2–E4、全年可行上界修复与技术排序继续阻断。
