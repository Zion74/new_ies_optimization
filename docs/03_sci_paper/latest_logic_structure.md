...
- E0-D-38 已在任何容量结果产生前冻结三状态时间聚合验证合同：实际基准；把实际热量等比例放大到 `H*=0.80` 且把 PCC 收紧到 `G*=0.70`；基准物理下把 BESS/TES 有效输出固定为 24 h。弃电帽锁定为实际全年可再生可用量的 10%，年度 PCC 目标只由真实 8784 h 无储能自然服务两阶段产生，并原样用于代表期与全年。每个状态先做四架构代表期规划，再做固定容量全年回代和三种储能架构全年重优化；当前仅合同冻结，尚未运行 D38；
- 原始证据审计确认杨凌表内没有供回水温度、抽汽温压或可直接识别三罐逐时损失/泵耗的设备参数。MT 继续使用 0.25/0.50/0.75 作者显热分割；E0-D-9B-1/2 数值均登记为作者敏感性。Guccione `140 EUR/kWe` 仍缺报价价格年，且罐、循环、三条换热/蒸汽发生路径、power-block retrofit、项目附加费和寿命项也未闭合；DLR `20–22 EUR_2020/kWh_th-net` 仅为两罐 Solar Salt 工程聚合锚点。本地 `price_sell/price_buy` 是脚本生成情景，不是杨凌结算；总排放乘碳价也不是配额履约成本。D34–D37 已关闭完整内生容量实现、工程材料性、代表周数据构造和分块状态边界缺口；D38 只完成结果前合同冻结，尚未完成求解与全年回代。TES/系统正式 TAC、D38/正式 E5 全年回代及 336 h 替代调度宽外界继续阻断正式结论；当前下一步只允许运行三个预验证状态，不得启动 E2 正式经济前沿或批量杨凌赢家扫描。D33–D38 合同见对应 `e0_public_*` 与 `e0_d34_*` 至 `e0_d38_*` 文件。
