...
- 原始证据审计确认杨凌表内仍缺项目级 TES 参数和完整账户。D41/D42 的 BESS 下界不是原 MILP 可行解、容量方案或项目 TAC。当前仍没有三架构可比的容量、成本、gap 或赢家。正式 E2–E4、全年上界修复与技术排序继续阻断；这不证明 TES 不可行或 BESS 更优。下一步必须另立结果前的严格证书恢复合同，不能在 D42 名下延时或改规则。
