...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44/D47 已使三架构下界闭合。D46/D48-R1/D49/D50 的成功上界恢复数均为 0。D50 在阶段 `3` 形成当前无回退块路径死路并以 `block_path_no_incumbent` 关闭；不证明原模型或 BESS 不可行。连续 guide、下界、24 h toy、Gate A、部分轨迹和阶段失败都不是正式可行容量或项目 TAC；正式项目账户完成前不能技术排序 |
