...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44 已恢复 TES R0/R1 下界 `254,860,566.6193158889 CNY`；BESS 下界仍为 `1,144,950,604.8368804 CNY`。D45 Hybrid R0 唯一正式运行的双证书仅完成 `20/24` / `16/24` 块，总状态 `no_strict_certificate`。D47 的 56 加权持久块实现与 OpenBayes Gate A 已通过，唯一正式证书尚未启动。已有下界都不是原 MILP 上界、容量或项目 TAC，不能技术排序；Hybrid 下界、真实项目账户、三架构可行上界和正式 TAC 仍未完成 |
...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43 唯一正式复算失败且不得重跑。D44 已恢复 TES 下界；D45 唯一正式 Hybrid R0 为 `no_strict_certificate` 且不得重跑。D47 新严格分解合同、实现与 Gate A 已闭合，下一步只允许唯一正式证书运行；D46 可行上界修复、技术排序和批量扫描仍禁止 |
...
| `src/tes_bess_boundary/e0d47_hybrid_weighted_persistent_certificate.py`、`tests/test_e0d47_hybrid_weighted_persistent_certificate.py`、`docs/03_sci_paper/e0_d47_hybrid_weighted_persistent_certificate_contract.md`、`数据采集/e0d47_gate_a/` | 只读 D45 同哈希 LP/dual，以 (1+nnz_j) 权重形成 56 个连续块，56-worker 顺序执行 IPX→必要时 simplex_1，逐块原子持久化并以 80 位向外舍入完整汇总 | E0/E5 | 第 1–11 节结果前冻结；源码/测试提交 `1515eca`，OpenBayes Gate A 为 `37/168/622 passed`、零失败/零跳过，manifest `80591c66...`。当前只开放唯一正式 D47，尚不产生 Hybrid 下界；D46 继续保留且不获准 |
