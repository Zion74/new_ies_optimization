...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43 本地实现门已通过：15 项新增、D40–D43 80 项、全包 534 项与 Ruff 通过；正式 dual 尚未读取。下一步只做提交与 OpenBayes 同哈希回归；Hybrid、上界修复和批量扫描仍禁止 |
