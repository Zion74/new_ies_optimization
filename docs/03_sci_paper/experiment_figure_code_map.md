...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46 上界恢复数为 0。D48-R1 已运行到 Hybrid 候选阶段；BESS/TES 均为 `no_primal_status_closure` 且没有上界，正式总 manifest 尚未形成，技术排序和批量扫描仍禁止 |
...
| `docs/03_sci_paper/e0_d48_hamming_feasibility_primal_recovery_contract.md`、`e0_d48_r1_administrative_path_correction_contract.md`、`数据采集/e0d48_gate_a_1090cd8/`、`数据采集/e0d48_invalid_wrong_output_path_launch/`、`数据采集/e0d48_hamming_feasibility_primal_recovery/` | D46 guide seed 只读诊断；原容量边界下完整二元等权 Hamming 搜索，再固定全部二元恢复原成本 LP | E0/E5 | 实现 `1090cd83...`、Gate A `1d894652...` 已归档。D48-R1 正确路径总批次正在运行；BESS/TES 两阶段各四文件已回传同哈希，均在硬墙钟内无 candidate/repair/上界，科学状态分别为 `no_primal_status_closure`；Hybrid 正在运行，当前本地目录仍是阶段副本 |
