...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43 唯一正式复算失败且不得重跑。D44 已恢复 TES 下界；D45 唯一正式 Hybrid R0 为 `no_strict_certificate` 且不得重跑。D47 新严格分解合同已结果前冻结，下一步只允许代码/Gate A；D46 可行上界修复、技术排序和批量扫描仍禁止 |
...
| `docs/03_sci_paper/e0_d47_hybrid_weighted_persistent_certificate_contract.md` | 只读 D45 同哈希 LP/dual，以 (1+nnz_j) 权重形成 56 个连续块，56-worker 顺序执行 IPX→必要时 simplex_1，逐块原子持久化并以 80 位向外舍入完整汇总 | E0/E5 | 第 1–11 节已在代码前冻结；D46 继续保留且不获准。D47 源码、测试、Gate A 和正式证据尚未实现，当前不产生 Hybrid 下界 |
