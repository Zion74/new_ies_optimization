...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46、D48-R1、D49 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46/D48-R1/D49 上界恢复数均为 0。D50 实现与 Gate A 已闭合，下一步只允许唯一正式 BESS 流水线；技术排序和批量扫描仍禁止 |
...
| `src/tes_bess_boundary/e0d50_full_year_coupled_physical_block_relax_and_fix.py`、`e0d50_monitored_executor.py`、两份 `tests/test_e0d50_*.py`、D50 合同、`数据采集/e0d50_gate_a_*` | BESS 完整 8784 h 年度耦合模型上的燃料编码持续投影、`336 h` 物理整数前视、`168 h` 逐块提交、完整物理轨迹提升、clean 原成本 LP 与单 child 阶段/资源编排 | E0/E5 | 第 1–11 节结果前冻结；实现提交 `04e17643...`，Gate A `d74891b9...` 为 `15/244/688 passed`、零跳过且未调用正式优化。中间阶段与部分轨迹均为 candidate-only；当前只允许唯一正式 BESS 流水线，尚无上界数值 |
