...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46、D48-R1 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46/D48-R1 上界恢复数均为 0。D49 Gate A `11b283d6...` 已通过，下一步只开放一次 BESS 方法门；正式结果尚不存在，技术排序和批量扫描仍禁止 |
...
| `src/tes_bess_boundary/e0d49_physics_first_fuel_projection_primal_recovery.py`、`e0d49_monitored_executor.py`、两份 `tests/test_e0d49_*.py`、`docs/03_sci_paper/e0_d49_physics_first_fuel_projection_primal_recovery_contract.md` | 保留物理二元的等权 Hamming、只投影 CHP 燃料编码位、全部结点/分段静态证明、逐时确定性精确提升、完整二元固定、原成本 LP 修复与 BESS-only 资源编排 | E0/E5 | 结果前合同已冻结；实现提交 `86a8b80e...`，本地 D49 定向 `14 passed`，三架构 24 h 完整链通过。OpenBayes Gate A 与正式 BESS 尚未执行，当前不产生上界、gap、TAC 或技术排序 |
