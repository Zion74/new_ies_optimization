...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书及影子成本稳健性、HiGHS 状态与求解误差 | E0-D-19/20 已闭合同 PCC 燃料空间和四账户证据门；E0-D-21 已给出不利遗漏成本的精确/区间翻转阈值，但不估计项目成本。TES 12 账户、完整 TAC 和容量规划仍未闭合 |
...
| `tests/` | 真实数据、本构、适配/桥接、线性、四架构、HiGHS、寿命、TES 温区/拓扑/夹点/MT/损失辅机、成本证据、BESS 正式账本、TES 正式就绪度、盈亏平衡、E0-D-17–D-21 回归 | E0 | 本地 `303 passed in 49.52s`；OpenBayes `303 passed in 26.34s`；关闭 pytest cache |
...
- 完整 E0 当前回归：本地 Python 3.11 为 `303 passed in 49.52s`；OpenBayes Python 3.10.18 为 `303 passed in 26.34s`；E0-D-21 代码、测试、thresholds/stress CSV 与 manifest 跨平台逐字节一致；
...
- 凭据与密码禁止写入仓库、配置或日志。
- 最新版代码和最小必要杨凌原始数据位于 `/root/e0-b-20260711-019f4f64/`；正式 E0-B 三文件位于其 `formal_data/e0b_formal_2024/`，E0-C 证据位于 `formal_data/e0c_heat_demand_adapter/`，D21 独立再生成件位于 `e0d21_shadow_cost_robustness_remote/`；E0-D-21 仅需 D19 canonical 与 D20 门控，不上传凭据和非必要保密资料。
