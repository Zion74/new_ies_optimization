...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46 上界恢复数为 0。D48-R1 已完成且三架构均为 `no_primal_status_closure`，上界恢复数仍为 0，总 manifest `ca024880...`；技术排序和批量扫描仍禁止 |
