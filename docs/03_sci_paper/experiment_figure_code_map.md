...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44/D47 已使三架构下界闭合。D46/D48-R1/D49/D50 的成功上界恢复数均为 0。D50 在阶段 `3` 形成当前无回退块路径死路并关闭；D51 以 `15/249/703 passed` 和 24 h 三检查点 clean 重放通过控制器 Gate 0。D52 实现提交 `c3b2e0cf...` 的 OpenBayes Gate A 已以 `17/266/720 passed`、零跳过闭合；唯一正式 BESS 正在运行，阶段 0/1 checkpoint 已通过完整变量映射和提交审计并进入阶段 2，但均不具正式上界资格，TES/Hybrid 禁止。连续 guide、下界、toy、Gate 0/Gate A、合同、部分轨迹、checkpoint 和阶段失败都不是正式可行容量或项目 TAC；正式项目账户完成前不能技术排序 |
...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46、D48-R1、D49、D50 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46/D48-R1/D49/D50 上界恢复数均为 0，技术排序和批量扫描仍禁止 |
...
| `tests/` | 真实数据、本构、适配/桥接、线性、四架构、HiGHS、寿命、TES 温区/拓扑/夹点/MT/损失辅机、成本证据、BESS 正式账本、TES 正式就绪度、盈亏平衡、E0-D-17–D43 执行与汇编回归 | E0 | Windows 完整回归 `534 passed in 62.51s`、D40–D43 `80 passed in 5.81s`；OpenBayes 完整回归 `534 passed in 34.18s`、D40–D43 `80 passed in 0.75s`；关闭 pytest cache；求解器仅 HiGHS，D43 测试不调用求解器 |
...
| `src/tes_bess_boundary/e0d40_gate_b_solver.py`、`tests/test_e0d40_gate_b_solver.py` | Gate A/服务哈希锁、父子进程资源监控、HiGHS bounds/incumbent、服务/守恒审计、预注册 gap 分类和三案汇总 | E0/E5 | 双端测试通过；唯一一次 BESS 60 s 预检完成且无 incumbent、资源门通过。正式 BESS 暴露父进程缺少硬墙钟，`4527.395 s` 后无结果并分类 `monolithic_not_viable`；后续合同须增加硬墙钟与严格分解证书 |
...
| `src/tes_bess_boundary/e0d41_strict_full_year_decomposition.py`、`tests/test_e0d41_strict_full_year_decomposition.py`、`数据采集/e0d41_strict_full_year_decomposition/` | 二元变量全覆盖分类、R0/R1 域变换、完整轨迹固定、三架构全年 Gate A/B 证据 | E0/E5 | Gate A 三架构通过；Gate B BESS 下界通过、TES R0 硬墙钟失败、Hybrid 未启动；总 manifest SHA-256 `bbc0638470859a58fe26a3166ec4825f455fd27671b7edf234b6e51557ee8aef` |
| `src/tes_bess_boundary/e0d41_gate_b_lower_bound.py`、`tests/test_e0d41_gate_b_lower_bound.py` | R0/R1 全年 HiGHS dual、独立硬墙钟、5 s 心跳、进程树资源门、目标/方向审计、R1 候选轨迹与架构汇总 | E0/E5 | BESS R0/R1 均通过，严格下界 `1,144,950,604.8368804 CNY`；TES R0 在 `720.462 s` 无返回合法 dual，TES R1/Hybrid 停止；不能技术排序 |
| `src/tes_bess_boundary/e0d41_gate_b_bundle.py`、`tests/test_e0d41_gate_b_bundle.py` | 只读核验逐架构 manifest，执行 BESS→TES→Hybrid 串行停止与最弱案例规则，确定性生成总 manifest/execution | E0/E5 | 提交 `0fb9346` 后正式汇编；总状态 `no_strict_certificate`，Gate C/D 权限为 false，execution SHA-256 `0b71fc77d7aa4faaad3b84f294faddd035dc8ea66df744df1ba27164c247af19` |
| `docs/03_sci_paper/e0_d41_strict_full_year_bound_repair_decomposition_contract.md` | 全年合法松弛下界、候选离散轨迹、原始全年可行修复上界、父进程硬墙钟与 gap 分类 | E0/E5 | 第 1–10 节结果前冻结；第 17 节登记 Gate B 最弱案例失败。D41 已停止，下一步另立 D42，不在 D41 名下延时或启动 Gate C/D |
| `src/tes_bess_boundary/e0d42_native_highs_certificate.py`、`tests/test_e0d42_native_highs_certificate.py` | 完整原生 LP 指纹、显式 presolve、callback、basis 检查点和 80 位定向舍入拉格朗日证书核心 | E0/E5 | 双端哈希一致；Gate A 正式结构门已通过；尚无正式全年下界 |
| `src/tes_bess_boundary/e0d42_full_year_structure_gate.py`、`tests/test_e0d42_full_year_structure_gate.py`、`数据采集/e0d42_native_highs_lagrangian_bound/` | TES R0/R1 与 Hybrid R0/R1 两分支 clean-process 重建、D41 结构锁、原始/预求解指纹及五案例汇编 | E0/E5 | 五案均通过；TES 双指纹相同、Hybrid 0/1 分支完整，`formal_gate_b_permitted=true`、`technical_ranking_permitted=false` |
...
| `src/tes_bess_boundary/e0d45_hybrid_r0_strict_lower_bound.py`、`tests/test_e0d45_hybrid_r0_strict_lower_bound.py`、`docs/03_sci_paper/e0_d45_hybrid_r0_strict_lower_bound_contract.md`、`数据采集/e0d45_gate_a/`、`数据采集/e0d45_hybrid_r0_strict_lower_bound/` | 锁定 Hybrid R0 原始/presolve 指纹，IPX/simplex_1 并行落盘完整 row-dual 快照，再复用 D44 的 24 块、80 位 fork 核形成严格下界；父进程强制总/阶段资源门、artifact 哈希和残留进程清理 | E0/E5 | 第 1–11 节结果前冻结；源码/测试提交 `270b04d`，OpenBayes Gate A 为 `27/131/585 passed`。唯一正式运行的双证书只完成 `20/24`、`16/24` 块，总 manifest `668fb0ea...`，状态 `no_strict_certificate`；D45 不得重跑，未生成 Hybrid 下界、可行上界、容量、gap、TAC 或技术排序 |
| `src/tes_bess_boundary/e0d47_hybrid_weighted_persistent_certificate.py`、`tests/test_e0d47_hybrid_weighted_persistent_certificate.py`、`docs/03_sci_paper/e0_d47_hybrid_weighted_persistent_certificate_contract.md`、`数据采集/e0d47_gate_a/`、`数据采集/e0d47_hybrid_weighted_persistent_certificate/` | 只读 D45 同哈希 LP/dual，以 (1+nnz_j) 权重形成 56 个连续块，56-worker 顺序执行 IPX→必要时 simplex_1，逐块原子持久化并以 80 位向外舍入完整汇总 | E0/E5 | 第 1–11 节结果前冻结；源码/测试提交 `1515eca`，OpenBayes Gate A 为 `37/168/622 passed`。唯一正式 IPX 56/56 块恢复 Hybrid R0 下界 `232,011,577.8359315691 CNY`，总 manifest `8b74c404...`，simplex_1 未启动，D47 不得重跑；D46 仅获另立合同权限 |
...
| `src/tes_bess_boundary/e0d50_full_year_coupled_physical_block_relax_and_fix.py`、`e0d50_monitored_executor.py`、两份 `tests/test_e0d50_*.py`、D50 合同、`数据采集/e0d50_gate_a_*`、`数据采集/e0d50_full_year_coupled_physical_block_relax_and_fix/` | BESS 完整 8784 h 年度耦合模型上的燃料编码持续投影、`336 h` 物理整数前视、`168 h` 逐块提交、完整物理轨迹提升、clean 原成本 LP 与单 child 阶段/资源编排 | E0/E5 | 实现 `04e17643...`、Gate A `d74891b9...` 与唯一正式运行已闭合。阶段 `0/1/2` 提交，阶段 `3` 无 incumbent，终态 `block_path_no_incumbent`；formal manifest `3efdbba5...`，没有 candidate/lift/repair/上界，D50 不得原样重跑 |
| `src/tes_bess_boundary/e0d52_full_year_checkpointed_bounded_backtracking_primal_recovery.py`、`e0d52_monitored_executor.py`、两份 `tests/test_e0d52_*.py`、D52 合同、`数据采集/e0d52_gate_a_*`、`数据采集/e0d52_full_year_checkpointed_bounded_backtracking_primal_recovery_live/` | D50 同一完整年度模型上的原子检查点、rollback 后 clean 重建/父前缀重放、顺序 no-good cut、一步有界回退、精确燃料提升、clean 原成本 repair、BESS-only 资源/哈希/Gate A 编排 | E0/E5 | 实现 `c3b2e0cf...`；OpenBayes Gate A 为 `17/266/720 passed`、零跳过，manifest `e5919147...`。唯一正式 BESS 已启动；阶段 0/1 checkpoint `9af8c1c5...` / `c8328947...` 捕获完整变量映射并累计固定 `1,009` 个物理二元，随后进入阶段 2。两个 checkpoint 均不具上界资格；完整 candidate、capacity、upper bound、gap 和技术排序资格仍未形成 |
