...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44 已恢复 TES R0/R1 下界 `254,860,566.6193158889 CNY`；BESS 下界仍为 `1,144,950,604.8368804 CNY`。D47 唯一正式 IPX 56/56 加权持久块已恢复 Hybrid R0 下界 `232,011,577.8359315691 CNY`，并覆盖 R1/原 MILP。D46 唯一正式总批次已完成，三架构均为 `no_candidate_incumbent`，未进入 Repair A/B，上界恢复数为 0。三个连续 guide、三架构下界和 24 h toy upper bound 都不是正式 8784 h 可行容量或项目 TAC，不能技术排序；真实项目账户、新的结果前可行性恢复路线和正式 TAC 仍未完成 |
...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46 唯一正式总批次没有任何 candidate incumbent 或 Repair，正式上界恢复数为 0。下一步须另立结果前可行性恢复合同；技术排序和批量扫描仍禁止 |
