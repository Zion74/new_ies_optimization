...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43 唯一正式复算失败且不得重跑。D44 Gate A 与唯一 Gate B 已完成；IPX 24/24 块证书恢复 TES 下界，总 manifest `d6fe2f34...`。D45 Hybrid R0 已通过 OpenBayes 同哈希 Gate A，下一步只允许唯一正式 Hybrid R0；可行上界修复、技术排序和批量扫描仍禁止 |
...
| `src/tes_bess_boundary/e0d45_hybrid_r0_strict_lower_bound.py`、`tests/test_e0d45_hybrid_r0_strict_lower_bound.py`、`docs/03_sci_paper/e0_d45_hybrid_r0_strict_lower_bound_contract.md`、`数据采集/e0d45_gate_a/` | 锁定 Hybrid R0 原始/presolve 指纹，IPX/simplex_1 并行落盘完整 row-dual 快照，再复用 D44 的 24 块、80 位 fork 核形成严格下界；父进程强制总/阶段资源门、artifact 哈希和残留进程清理 | E0/E5 | 第 1–11 节结果前冻结；源码/测试提交 `270b04d`，SHA-256 `cf977561...` / `8e6b5985...`。OpenBayes Gate A 为 `27/131/585 passed`、零跳过，manifest SHA-256 `570b801c...`；正式 Hybrid 尚未执行，R0 下界不生成可行上界、容量、gap、TAC 或技术排序 |
