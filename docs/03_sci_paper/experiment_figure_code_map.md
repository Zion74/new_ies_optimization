...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44/D47 已使三架构下界闭合。D46/D48-R1/D49/D50/D52 的成功上界恢复数均为 0。D52 阶段 0/1 checkpoint 通过审计，但阶段 2 attempt 触发冻结硬墙钟，终态 `no_primal_status_closure`，不是内存耗尽且不证明 BESS 不可行。连续 guide、下界、toy、Gate 0/Gate A、合同、部分轨迹、checkpoint 和阶段失败都不是正式可行容量或项目 TAC；正式项目账户完成前不能技术排序 |
...
| `src/tes_bess_boundary/e0d52_full_year_checkpointed_bounded_backtracking_primal_recovery.py`、`e0d52_monitored_executor.py`、两份 `tests/test_e0d52_*.py`、D52 合同、`数据采集/e0d52_gate_a_*`、`数据采集/e0d52_full_year_checkpointed_bounded_backtracking_primal_recovery/` | D50 同一完整年度模型上的原子检查点、rollback 后 clean 重建/父前缀重放、顺序 no-good cut、一步有界回退、精确燃料提升、clean 原成本 repair、BESS-only 资源/哈希/Gate A 编排 | E0/E5 | 实现 `c3b2e0cf...`、Gate A `e5919147...`。唯一正式 BESS 阶段 0/1 checkpoint `9af8c1c5...` / `c8328947...` 累计固定 `1,009` 个物理二元；阶段 2 attempt 触发 `390 s` 硬墙钟，终态 `no_primal_status_closure`，formal manifest `c80fc8bf...`。没有完整 candidate、capacity、upper bound、gap 或技术排序资格，D52 不得恢复或原样重跑 |
