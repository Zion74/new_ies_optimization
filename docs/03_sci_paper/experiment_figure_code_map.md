...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44/D47 已使三架构下界闭合。D46/D48-R1/D49/D50 的成功上界恢复数均为 0。D50 在阶段 `3` 形成当前无回退块路径死路并关闭；D51 以 `15/249/703 passed` 和 24 h 三检查点 clean 重放通过控制器 Gate 0。D52 实现提交 `c3b2e0cf...` 的 OpenBayes Gate A 已以 `17/266/720 passed`、零跳过闭合，8784 h build-only 未调用 solver；现只授权 BESS 正式运行，TES/Hybrid 禁止。正式目录尚未创建，也没有正式图表、容量或上界资格。连续 guide、下界、toy、Gate 0/Gate A、合同、部分轨迹和阶段失败都不是正式可行容量或项目 TAC；正式项目账户完成前不能技术排序 |
...
| `src/tes_bess_boundary/e0d52_full_year_checkpointed_bounded_backtracking_primal_recovery.py`、`e0d52_monitored_executor.py`、两份 `tests/test_e0d52_*.py`、D52 合同、`数据采集/e0d52_gate_a_*` | D50 同一完整年度模型上的原子检查点、rollback 后 clean 重建/父前缀重放、顺序 no-good cut、一步有界回退、精确燃料提升、clean 原成本 repair、BESS-only 资源/哈希/Gate A 编排 | E0/E5 | 实现 `c3b2e0cf...`；OpenBayes Gate A 为 `17/266/720 passed`、零跳过，全年 build-only 规模 `597,318/79,057/527,053` 且未调用 solver，manifest `e5919147...`。只允许唯一 BESS 正式运行；正式目录与年度 optimize 尚未产生，没有正式 candidate、capacity、upper bound、gap 或技术排序资格 |
