...
| 编号 | 目的 | 核心设置 | 主输出 | 代码状态 |
|---|---|---|---|---|
| E0 | 验证数据、物理与 MILP | 双机 CHP、BESS、HT/MT TES、PCC、寿命成本 | 可行域、能量守恒、现金流审计、TES 证据/成本门、BESS 正式账本、同 PCC 服务 EAC 上限、非燃料成本证书、影子成本稳健性、逐时 PCC、替代调度包络、严格数值证书、16 账户 TAC 路线、项目取证接口、公开敏感性成本账、完整内生容量接缝、工程材料性门、代表周数据门、分块循环状态边界、全年直接求解资源与精度门、HiGHS 状态与求解误差、严格全年下界—可行上界证书 | D38–D43 失败均已登记。D44/D47 已使 BESS、TES、Hybrid 三架构下界闭合。D46 唯一正式总批次三架构均为 `no_candidate_incumbent`，上界恢复数为 0；只读诊断已定位联合离散轨迹问题。D48 结果前合同已冻结“原容量边界 + 完整二元等权 Hamming 搜索 + 固定二元原成本修复”，但源码、Gate A 与正式批次均未启动。连续 guide、三架构下界和 24 h toy upper bound 都不是正式 8784 h 可行容量或项目 TAC；D48 正式证据和项目账户完成前不能技术排序 |
...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46 上界恢复数为 0。D48 结果前合同已经冻结，但尚未实现或运行；只有同哈希 Gate A 后才允许一次正式 Hamming primal 恢复批次，技术排序和批量扫描仍禁止 |
...
| `docs/03_sci_paper/e0_d48_hamming_feasibility_primal_recovery_contract.md`、`数据采集/e0d46_postmortem_seed_violation_diagnostic/` | D46 guide seed 只读逐约束诊断；撤销最大容量固定、恢复原容量边界，以完整二元等权 Hamming 目标搜索首个整数可行轨迹，再固定全部二元恢复原成本 LP | E0/E5 | D46 postmortem bundle `c74a694...` 已归档且 `solver_invoked=false`；D48 第 1–11 节结果前冻结，源码、测试、Gate A 和正式批次均待实现，尚无上界或新求解结论 |
