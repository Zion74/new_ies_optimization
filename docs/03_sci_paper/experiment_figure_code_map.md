...
| `src/tes_bess_boundary/e0d42_native_highs_certificate.py`、`tests/test_e0d42_native_highs_certificate.py` | 完整原生 LP 指纹、显式 presolve、callback、basis 检查点和 80 位定向舍入拉格朗日证书核心 | E0/E5 | 双端哈希一致；原生证书定向 11 项通过。与结构审计器合计 D42 定向 16 项、D40–D42 定向 53 项、全包 507 项；尚未形成真实全年结构或下界证据 |
| `src/tes_bess_boundary/e0d42_full_year_structure_gate.py`、`tests/test_e0d42_full_year_structure_gate.py` | TES R0/R1 与 Hybrid R0/R1 两分支 clean-process 重建、D41 结构锁、原始/预求解指纹及五案例汇编 | E0/E5 | 代码先于结果固定为 `80b5b5c`；双端 507 项回归通过。真实五案例尚未执行，当前不开放 Gate B |
