...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D43、D45、D47、D46、D48-R1 均不得原样重跑。D44/D47 已使三架构严格下界闭合；D46/D48-R1 上界恢复数均为 0。D49 合同只开放 Gate A 和一次 BESS 方法门，正式结果尚不存在；技术排序和批量扫描仍禁止 |
