...
| E5 | 全年证据与时间方法验证 | 真实 8784 h 单循环块；D36/D39 仅保留为失败对照或候选生成 | 全年可计算性、严格 gap、分解证书、代表期误差 | D42 TES R0 为 `no_strict_certificate`，按合同停止且 Hybrid 未启动；下一步须另立结果前的证书恢复合同，不得恢复代表期主证据、启动上界修复或批量扫描；`_ch4_p3_typdays.py` 仅作旧原型 |
