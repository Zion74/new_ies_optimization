...
状态：**物理容量账本、部件—容量基准、三状态罐完整库存认证、温区覆盖和生命周期 portfolio 绑定已经实现；E0-D-10 完成价格年/分母/技术边界缺口矩阵及可执行认证门，E0-D-11 又加入 NREL BESS 敏感性专用成本账本及 FOM/替换防双计。本地/OpenBayes 全量 258 项通过；TES 与 Energy+ 正式成本数值仍未闭合。**
...
电加热器按 `€/kWel`、蒸汽发生器按 `€/kWth` 的端口映射与本合同一致。`21 €/kWhth` 两罐储热系统包含热罐、冷罐、盐、基础和电气/管阀等成本份额，是系统聚合锚点，只能用于校准分项 ledger 总量，不能再与盐和罐分项叠加。其 560/620 °C 方案可为高温设备提供温区证据，但冷端、蒸汽参数和规模仍须与杨凌候选方案逐项核对。
...
这些门槛完成前，E0-D-5 只证明“单位、容量和温区不会静默错接”，不提供正式 TAC，也不启动 E1–E6 批量实验。
