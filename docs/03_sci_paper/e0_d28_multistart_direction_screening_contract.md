...
状态：**336 h 的 `negated` 与 `alternating` 两个预注册方向已在 OpenBayes 完成。两条一步筛查均未找到高于 D27 的新可行 L1 证人；D28 当次运行未改变 D27 区间，且只生成可行 L1 下界，不生成全局上界。后续 D29 在保留下界的同时把全局上界收紧为 `845,052.030831 MWh/a`，D30 再收紧为 `777,141.368858 MWh/a`。**
