...
E0-D-40 已在任何新模型规模或求解结果出现前冻结真实 8784 h 全年优先路线。当前实现只提供无代表期 provenance 的服务提取、四架构 clean-process build-only 审计、完整容量上界/安装联动、线性扫描、单全年循环边界和 20 GiB/40 GiB 资源汇总；不创建求解器。源码/可移植测试 SHA-256 分别为 `1110af20f9b300f171dfbd5c21882822f2fa932908dbced65cbfaee8dcc30f6a` 与 `bfc92d34b2e55899619708ca02ccd158463c368d94bc2943b2b0afeafcb1f7f4`。Windows 定向测试为 `6 passed`；完整回归在夹具可移植修订前为 `460 passed`，OpenBayes 首轮同步回归暴露 3 项本地证据路径夹具错误、其余 `457 passed`，未启动 Gate A。夹具已改为自包含合成 JSON，待服务器复跑。
