...
3. 下一主门是在任何新结果前冻结新的代表期修订合同：明确利用第 49/16 周等失败诊断的选择规则、代表周预算、52 周重分配、文件隔离和 baseline gate-first 验收；只有新 baseline 代表期—全年可行性一致后，才允许恢复三状态四架构预验证；
