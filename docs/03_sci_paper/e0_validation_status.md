...
| 环境 | 范围 | 结果 |
|---|---|---|
| Windows `.venv-e0` | 全包；含 D33–D38-R1、bundle auditor、周级失败诊断与 HiGHS | `445 passed in 115.63s`（关闭 pytest cache） |
| OpenBayes Python 3.10.18 隔离环境 | 全包；含正式数据、E0-D-17–D38-R1、bundle auditor、周级失败诊断和 HiGHS | `445 passed in 31.64s`（关闭 pytest cache） |
...
E0-D-39 已在任何新数据或结果产生前把失败诊断转化为一次性、可审计修订：原六周加第 49/16 周，52 周沿用 D36 的 672 维距离重分配，年尾 72 h 与所有物理/服务/成本规则不变。Gate B 预先要求 D39 代表期与真实全年对 no-storage 10% 服务的可行性分类一致，且自然最小弃电率误差不超过 1 个百分点；否则 D39 失败，不继续加周或放宽阈值。当前尚无 D39 数据与结果。
...
3. D39 结果前合同已冻结；下一步实现 Gate A 数据构造并在 Windows/OpenBayes 做逐字节复现，再执行 Gate B baseline 零燃料保真门。只有 Gate A/B/C 全部通过，才允许恢复 R1 三状态四架构预验证；
