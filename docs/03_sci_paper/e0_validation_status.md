...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43、D45、D47 与 D46 均不得原样重跑；D44 已恢复 TES R0/R1 合法下界，D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。D46 唯一正式总批次的三架构均未产生 incumbent，上界恢复数为 0；后续若继续全年路线，必须先另立结果前可行性恢复合同，不能在 D46 名下事后更换 seed、容量锚点或墙钟；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44 已恢复 TES R0/R1 严格下界；D45 唯一正式 Hybrid R0 在完整 dual 之后因两条 24 块证书均触发墙钟而没有正式证书，且不得重跑。D47 已以 56/56 个加权持久块恢复 Hybrid R0 严格下界 `232,011,577.8359315691 CNY`，三架构下界门闭合。D46 唯一正式批次已完成，但 BESS/TES/Hybrid 均为 `no_candidate_incumbent`，没有 Repair A/B、正式可行容量、上界或 gap。杨凌正式 E2 经济结论继续等待 D24/D25 项目证据与新的结果前可行性恢复路线；在可行容量、gap 与 TAC 形成前，699 次边界扫描继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
