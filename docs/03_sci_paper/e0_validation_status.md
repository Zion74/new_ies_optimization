...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43 不得重跑；D44 已恢复 TES R0/R1 合法下界。下一步先另立并冻结 Hybrid 下界合同，不得在 D44 名下直接启动；全年可行上界修复、技术排序与批量扫描继续禁止；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44 已用冻结 IPX dual 恢复 TES R0/R1 严格下界，但这不是可行上界、容量或项目 TAC，也不能与 BESS 下界直接形成技术排序。杨凌正式 E2 经济结论继续等待 D24/D25、Hybrid 严格下界与三架构可行上界路线；699 次边界扫描继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
