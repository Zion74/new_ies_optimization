...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43 不得在原合同下重跑；Hybrid 与全年可行上界修复继续禁止；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。杨凌正式 E2 经济结论继续等待 D24/D25 与新的三架构严格全年证书路线。D43 已从两个 D42 冻结 dual 快照尝试恢复 TES 下界，但均在冻结硬墙钟前未完成严格认证；Hybrid 仍为空，699 次边界扫描继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
