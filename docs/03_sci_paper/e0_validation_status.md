...
- D48 Hamming 可行性—原成本修复：第 1–11 节已在任何实现和正式结果前冻结。候选阶段撤销 D46 最大容量固定、恢复原容量边界，只以完整二元清单的等权 Hamming 距离替换原成本目标；首个审计 incumbent 后重建原模型、固定完整二元并恢复原成本 LP，连续容量保持自由。源码、Gate A 和正式批次均未启动，当前不产生任何新数值结论。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43、D45、D47 与 D46 均不得原样重跑；D44 已恢复 TES R0/R1 合法下界，D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。D46 唯一正式总批次的三架构均未产生 incumbent，上界恢复数为 0。D48 结果前合同现已冻结；后续只允许先实现其等权 Hamming 可行性搜索、固定完整二元原成本修复和 Gate A，不能在 D46 名下事后更换 seed、容量锚点或墙钟；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44/D47 已使三架构下界门闭合。D46 唯一正式批次三架构均为 `no_candidate_incumbent`；postmortem 仅定位联合离散轨迹问题，不生成上界。D48 已冻结为下一道唯一全年可行性路线，但 Gate A 和正式批次尚未启动。杨凌正式 E2 经济结论继续等待 D24/D25 项目证据与 D48 正式证据；在可行容量、gap 与 TAC 形成前，699 次边界扫描继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
