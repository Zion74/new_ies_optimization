...
- D49 物理优先燃料投影—原成本修复：第 1–11 节结果前合同与源码/测试提交 `86a8b80e...` 已冻结。OpenBayes Gate A 已在同哈希 Linux ASCII 路径通过：D49 定向、D40–D49、全包为 `14/219/673 passed`，均零失败、零错误、零跳过；Ruff/pycompile 通过。BESS/TES/Hybrid 8784 h build-only 均投影 `52,704` 个燃料编码位，保留物理二元 `26,353/35,136/43,921`，且分区、依赖、约束身份、原容量边界和静态提升证明全部通过，`formal_optimization_invoked=false`。Gate A manifest/execution 为 `11b283d6...` / `2fd4a896...`，只授权一次 BESS 正式方法门；当前还没有 candidate、repair、容量或上界。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43、D45、D47、D46 与 D48-R1 均不得原样重跑；D44 已恢复 TES R0/R1 合法下界，D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。D46 三架构均无 incumbent；D48-R1 三架构均为 `no_primal_status_closure` 且上界恢复数为 0。D49 Gate A 已闭合，下一步只执行冻结参数的一次 BESS 正式候选—精确提升—原成本修复方法门；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44/D47 已使三架构下界门闭合。D46 三架构均无 incumbent；postmortem 不生成上界。D48-R1 三架构都未闭合 primal 状态，正式上界恢复数仍为 0。D49 合同、实现和 OpenBayes Gate A 已闭合，但正式 BESS 数值仍为空。杨凌正式 E2、699 次边界扫描、gap 与技术排序继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
