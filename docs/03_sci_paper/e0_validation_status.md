...
- D47 Hybrid 加权持久化证书：结果前合同、源码/测试与 OpenBayes 同哈希 Gate A 已闭合，只读 D45 同哈希 LP/dual，不调用求解器。源码/测试提交为 `1515eca`，SHA-256 为 `a503a8c0...` / `9347f4e5...`；Linux D47、D40–D47、全包为 `37/168/622 passed`，零失败、零跳过。正式 LP 的 56 块总工作权重 `2,525,502`，最小/最大 `43,901/46,293`、比值 `1.054486`；Gate A manifest `80591c66...`。当前只开放唯一一次正式运行，尚未形成 Hybrid 下界。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43 不得重跑；D44 已恢复 TES R0/R1 合法下界。D45 唯一正式 Hybrid R0 已以 `no_strict_certificate` 结束且不得重跑；D47 新严格分解合同、源码/测试与 Gate A 已闭合，下一步只允许唯一正式 D47；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44 已恢复 TES R0/R1 严格下界；D45 唯一正式 Hybrid R0 在完整 dual 之后因两条 24 块证书均触发墙钟而没有正式证书，且不得重跑。D47 的 56 加权持久块只读恢复合同、实现和 Gate A 已闭合，唯一正式运行尚未启动。杨凌正式 E2 经济结论继续等待 D24/D25、D47 恢复 Hybrid 严格下界并再取得三架构可行上界权限；699 次边界扫描继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
