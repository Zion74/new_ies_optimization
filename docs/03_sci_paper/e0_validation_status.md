...
D49 已完成合同、实现、OpenBayes Gate A 和唯一正式 BESS 方法门；正式候选在 `3720.637 s` 内没有 incumbent、exact lift、repair、容量或上界，终态为 `no_primal_status_closure`。D50 唯一正式 BESS 流水线也已关闭：阶段 `0/1/2` 捕获 incumbent 并累计固定 `1,513` 个物理二元，阶段 `3` 在当前已固定路径下返回 `Infeasible` 且无 incumbent，正式终态 `block_path_no_incumbent`。总 manifest `3efdbba5...`；没有完整物理轨迹、精确提升、clean repair、容量或上界。D46/D48-R1/D49/D50 的成功上界恢复数均为 0。
...
- D43 已按结果前合同完成唯一一次正式离线复算。两个 D42 solution 归档的完整 `439,018` 维 row dual 均通过哈希链与准入门；IPX/simplex 两个 clean child 均运行约 `1800.49 s` 后触发冻结硬墙钟，返回码 `-15`，没有 result/certificate。总 manifest SHA-256 为 `c7b7e42973c30778efb791e2369ec5dc60dd4c70c75db333bfb5d3e1ac8f4526`，状态 `no_strict_certificate`；该阶段 TES/Hybrid 尚无合法可比下界。
...
- D45 Hybrid R0：结果前合同、提交 `270b04d` 与 OpenBayes 同哈希 Gate A 均已闭合，唯一正式运行也已结束。prepare 身份与双 row-dual snapshot 均通过；IPX/simplex_1 的 24 块证书阶段在冻结 `900 s` 硬墙钟前分别完成 `20/24` 与 `16/24` 块，没有 certificate/result。总 manifest SHA-256 `668fb0ea4c9293f789781298ca54f56da2bdcb55a3a7806d5bf8171d6e24cc55`，状态 `no_strict_certificate`；峰值聚合 RSS `16.827 GiB`、最低可用内存 `87.735 GiB`，不是内存失败。D45 当时没有形成 Hybrid 严格下界，且不得重跑。
- D47 Hybrid 加权持久化证书：结果前合同、源码/测试与 OpenBayes 同哈希 Gate A 均已闭合，只读 D45 同哈希 LP/dual，不调用求解器。源码/测试提交为 `1515eca`，SHA-256 为 `a503a8c0...` / `9347f4e5...`；Linux D47、D40–D47、全包为 `37/168/622 passed`，零失败、零跳过。唯一正式 IPX phase 完成 56/56 块并恢复 Hybrid R0 严格下界 `232,011,577.8359315691 CNY`，simplex_1 按合同未启动；该数值也合法下界 Hybrid R1/原 MILP。总 manifest/execution 为 `8b74c404...` / `ed978c36...`，运行 `539.050 s`、峰值聚合 RSS `18.838 GiB`，非法端点与活动残留进程均为 0。正式包 72 个文件，checksum 清单覆盖其余 71 个并与远端同哈希。三架构现均有严格下界，D46 仅获得另立可行上界/修复合同的权限；仍无可行容量、项目 TAC、gap 或技术排序。
- D46 三架构可行上界与固定二元修复：第 1–11 节结果前合同、最终源码/测试提交 `4a18f42` 与 OpenBayes `22/204/644 passed` Gate A 均已闭合。唯一正式总批次运行 `8820.162 s`，总 manifest `8693722...`；BESS/TES/Hybrid 均为 `no_candidate_incumbent`，没有进入 Repair A/B，上界恢复数为 0。该失败不证明原物理模型不可行；没有正式可行容量、上界、gap、项目 TAC 或技术排序，D46 不得原样重跑。
...
- D50 全年耦合物理分块 Relax-and-Fix：结果前合同、实现 `04e17643...`、Gate A `d74891b9...` 与唯一正式 BESS 运行均已闭合。阶段 `0/1/2` 捕获 incumbent 并累计固定 `1,513` 个物理二元；阶段 `3` 在当前固定路径下返回 `Infeasible`、无 incumbent，正式状态 `block_path_no_incumbent`。候选/solver 总运行 `1218.705/703.379 s`，峰值聚合 RSS `3.607 GiB`、最低可用内存 `93.739 GiB`、残留进程 0。没有 candidate CSV、完整物理快照、精确提升、repair、容量、上界或 gap；只否定无回退贪心块路径，不证明原 MILP 或 BESS 不可行，D50 不得原样重跑。
...
D49 Gate A 证据位于 `风光火+熔盐储热/数据采集/e0d49_gate_a_86a8b80/` 与相邻 work 目录。Gate A manifest/execution SHA-256 分别为 `11b283d6825cd5fcc5b41a09b8400bdb6116bb11e830b1dd1bc42b9417e789dd` / `2fd4a89660ff6b8443832eb29891e8dec689601bf54ca6bd041387502c60792b`；checksum 清单覆盖 19 个 work 产物和 2 个 Gate A 产物，远端—本地复核零不一致。首次误传 D46 摘要 JSON 的 build CLI 被 guide 哈希门在 solver 调用前拒绝并保留日志；通过版本使用锁定 `*_guide.csv.gz`，没有改变代码、模型、种子、选项、容差或墙钟。

D49 正式证据位于 `风光火+熔盐储热/数据采集/e0d49_physics_first_fuel_projection_primal_recovery/`。formal/BESS/execution/heartbeat/空 solver log SHA-256 分别为 `0d66f06d...`、`7f86f9c4...`、`4979c845...`、`c226513f...`、`e3b0c442...`；规范 checksum 覆盖 5 个远端产物，formal manifest 声明的 4 个 artifact 也全部复核零不一致。`resource_gate_passed=false` 仅由父级硬墙钟受控停止触发，并非 RSS 或可用内存越界。该包不含 candidate 或 repair，只支持 `no_primal_status_closure` 结论。

D50 Gate A 证据位于 `风光火+熔盐储热/数据采集/e0d50_gate_a_work_04e1764/` 与 `e0d50_gate_a_04e1764/`。build-only 文件 SHA-256 `196c5fd1...`，53 块覆盖与阶段 `0/26/51/52` 域审计通过，D41 guide 命中 `2d03ab0a...` 且未写入模型。Gate A manifest/execution SHA-256 为 `d74891b9...` / `35827275...`；排除 checksum 自身后的 12 个规范文件远端—本地零不一致，清单 SHA-256 `6b1c3e06...`。该 Gate 只授予唯一正式 BESS 启动权限，不是候选或上界。

D50 唯一正式证据位于 `风光火+熔盐储热/数据采集/e0d50_full_year_coupled_physical_block_relax_and_fix/`。远端 11 个正式文件与 2 个 launcher 文件已全部回传；formal manifest 声明的 artifact 与本地逐项零哈希不一致，本地最终 `SHA256SUMS.txt` 覆盖 14 个非自身文件且零不一致，清单 SHA-256 `ff44043a...`。formal/bess manifest SHA-256 为 `3efdbba5...` / `ac8c9522...`；四个阶段文件分别为 `26f17de7...`、`cdc16ad1...`、`d8a1df9f...`、`b65ef6b4...`。阶段 01 检查点保留为过程证据，但最终 manifest 是唯一终态入口。
...
D52 已独立冻结全年 BESS 检查点化有界回退正式合同，并以实现提交 `c3b2e0cf67bb76f7dbb8ae4ac4e5b2017c3a616c` 完成独立核心、BESS-only 监控执行器、测试和 OpenBayes 同哈希 Gate A。实现逐字节沿用 D50 的真实 8784 h 模型和服务，将 `336 h` 整数前视、`168 h` 提交扩展为 52 个完整块加 48 h 年尾的 53 阶段；固定常数零可行性目标、一步回退、每阶段最多 3 个不同模式、全批最多 8 次回退和最多 69 次 solver 尝试，再执行精确燃料提升与 clean 原成本 repair。rollback 必须销毁当前模型、clean 重建并重放父检查点和顺序 cuts；初始与回退 clean build 均向父监控器发布 390 s 计时事件。HiGHS 12 线程、seed 0、每次 `360/390 s` 软/硬墙钟、总批次 `32,400 s` 和全部成功/失败终态均未改变。Gate A 的 D52 定向、D40–D52 兼容和全包回归为 `17/266/720 passed`，零失败零跳过；Ruff 与 `py_compile` 通过。8784 h build-only 审计得到 `597,318` 个活动变量、`79,057` 个二元变量、`527,053` 条活动约束和零非线性；Gate A manifest/execution SHA-256 为 `e591914704182f06d15b11b8a3116e358d85ec29db15a8b676d4e021e739f55c` / `432fcdeac603efbcb081e8dbd4f146e1951c273735e8934e555e9c1926b7bcb2`。唯一正式 BESS 中，阶段 0/1 分别在 `243.519/219.709 s` 内以 HiGHS `Optimal` 捕获完整变量映射，提交 `505/504` 个物理二元并累计固定 `1,009` 个；checkpoint/values SHA-256 为 `9af8c1c5...` / `84956758...` 与 `c8328947...` / `2b54ba90...`。阶段 2 attempt 0 未在冻结 `390 s` 父级硬墙钟前返回，父进程以 `SIGTERM`、返回码 `-15` 收口。总状态 `no_primal_status_closure`，formal manifest SHA-256 `c80fc8bffa5cb49478e552ef44b2eea1726660f3421d306ed68146e8f9bc0f73`，总运行 `1185.041 s`；峰值 child/聚合 RSS `3.445/3.471 GiB`、最低可用内存 `93.859 GiB`、RSS warning 为 false、残留进程 0，故不是内存耗尽。没有完整 candidate、精确提升、clean repair、容量、上界或 gap；两个 checkpoint 均 `formal_upper_bound_eligible=false`。`restart_or_resume_permitted=false`，D52 已关闭，TES/Hybrid 未执行且继续禁止。

## 2. 测试证据

| 环境 | 范围 | 结果 |
|---|---|---|
| Windows `.venv-e0` | 全包；含 E0-D-17–D43 只读离线证书编排器 | `534 passed in 62.51s`（关闭 pytest cache；D40–D43 定向 `80 passed in 5.81s`） |
| OpenBayes Python 3.10.18 隔离环境 | 全包；含正式数据、D41/D42 证据链与 D43 只读离线证书编排器 | `534 passed in 34.18s`（关闭 pytest cache；D40–D43 定向 `80 passed in 0.75s`） |
| OpenBayes Python 3.10.18 隔离环境，D51 实现提交 `baec961...` | D51 定向 / D40–D51 兼容 / 全包 | `15 / 249 / 703 passed`，三组均零失败、零错误、零跳过；Ruff 与 `py_compile` 通过 |

D26–D43 使用 `Pyomo 6.10.1`、`highspy 1.15.1`，正式求解器仅为 HiGHS；D37 结构审计和 D43 离线证书编排本身不调用求解器。OpenBayes 包路径为 `/root/e0-b-20260711-019f4f64/tes_bess_boundary`，正式数据合同仍为 `TES_BESS_E0B_FORMAL_DIR=/root/e0-b-20260711-019f4f64/formal_data/e0b_formal_2024`。D27、D28、D29 的规范汇总位于 `/root/e0-b-20260711-019f4f64/数据采集/` 下同名目录；D30 bounds-only/全局原始探针与规范汇总位于 `e0d30_physics_service_bound_tightening/`；D31 双窗口 OBBT、24 h 等价探针与负筛查证书位于 `e0d31_intertemporal_obbt/`；D32 双窗口分块屏幕、24 h reopened 等价探针位于 `e0d32_joint_block_envelope/`；D36 正式构造位于 `/root/e0-b-20260711-019f4f64/e0d36_representative_weeks/`；D37 结构审计位于 `/root/e0-b-20260711-019f4f64/e0d37_block_cyclic_boundaries/`。本轮只上传已提交的 D43 编排器与测试，未上传本地受限资料。
...
D43 新增源码/测试 SHA-256 为 `684385d5a33a531a9034f52ad755b7655adc2e58690ca689ad4e2f08eb889791` / `50c5e819f206e87aaba3f27254e401529140f4f6d6fa8660d5bc901afb691933`，已结果前提交为 `78e30ee`，OpenBayes 逐字节同哈希。实现固定 case/lp/phase execution、BESS reuse、structure manifest 与 solution 完整哈希链，执行 LP/数组/finite dual 准入、未修改 80 位证书器、双 clean child、父进程资源门、Decimal 选择和只读 README；正式字段永久禁止优化器与技术排序。本地新增 `15 passed`、D40–D43 `80 passed`、全包 `534 passed`，Ruff 通过；OpenBayes `py_compile`、D40–D43 `80 passed in 0.75s`、全包 `534 passed in 34.18s`。唯一一次正式 D43 随后读取两个冻结 dual；两 child 均在约 1800.49 s 硬墙钟前未完成 80 位证书，未生成 result/certificate。总 execution 记录峰值父子合计 RSS `0.7346572875976562 GiB`、最低可用内存 `96.6288833618164 GiB`，故不是内存失败；规范输出已下载并逐文件同哈希。

D44 源码/测试以提交 `b52c722` 和 SHA-256 `16786dd98757851dc2829b335d12ddb8dfeab38fd9bc03fcf3ac840e9df41c4c` / `eca55183bb8cd4e7eed133409556cdc2ad84ef1fca42b169406f459aa4f62a2e` 结果前冻结。OpenBayes Gate A 为 D44 `24 passed`、D40–D44 `104 passed`、全包 `558 passed`，零失败、零跳过；Gate A manifest SHA-256 为 `49b81748508bd517afa9c8b43fac4b3400d6844a0b294838010c10adac89f289`。唯一正式 Gate B 的两个 phase 均完成 24/24 块；IPX 覆盖全部 `509,289` 列和 `1,806,011` 个非零元、非法端点为 0，80 位 lower/upper 为 `254860566.61931588889075258309724606578637338890918249419801438224278086471875331` / `2091080840.3627077923443033647707256993514499305815595432283723297221797777319790 CNY`。simplex_1 有 `15,195` 个所需无穷端点，按合同不合格。总 manifest SHA-256 为 `d6fe2f34a354e5986ad4775034135f090df2e74492e0c7abc8f95861cb89739f`，execution SHA-256 为 `673f4442d1f53d714f5eabd0c450c33373457cbd214a5ce0a85956d60f89946e`；运行 `819.732 s`，峰值聚合 RSS `15.434 GiB`，无停止原因。25 个 Gate A/正式远端文件与本地逐文件同哈希。D44 只恢复 TES 下界并开放另立 Hybrid 下界合同；它没有调用优化器或 HiGHS optimize，也不允许技术排序。

D45 已按结果前合同实现并执行最弱完整 Hybrid R0 路线：锁定原始 LP 指纹 `3534a0c9...` 与 presolved 指纹 `756014ec...`，并行运行 12-thread IPX/simplex_1 生成完整 row dual，再以未修改 D44 核对两个快照执行 24 块/80 位 fork 认证。源码/测试以提交 `270b04d6c8e65bd67a3953db722a0c082e058fc5` 固定，SHA-256 为 `cf977561f6471fd99fb9c4d3eed4dc04b65277f7b8a10f3013d10bd5e4a0866d` / `8e6b598530a886073188cc60f3ecd6b4c8cbd2c9ffcd0e75c0b4b3595219fe33`；OpenBayes 同哈希 Gate A 的 D45、D40–D45 定向和全包回归为 `27/131/585 passed`，均零失败、零跳过，Gate A manifest 为 `570b801c...`。唯一正式运行中，prepare 的 16 项身份检查全部通过，两个 solution SHA-256 为 `eed2b064...` / `6f4d0276...`；certificate phase 各启用 24 个 fork worker，但 `900 s` 硬墙钟前 IPX/simplex_1 仅完成 `20/24` / `16/24` 块，均以 `SIGTERM` 收口。总 manifest/execution SHA-256 为 `668fb0ea4c9293f789781298ca54f56da2bdcb55a3a7806d5bf8171d6e24cc55` / `60af4ee5b16f9aed6ec1a048b87cd57cbaf58b9b90141001ad667bdc71dcbca0`，运行 `1969.958 s`，终态 `no_strict_certificate`。29 个正式文件与本地副本逐文件同哈希，结束后残留进程为 0；D45 本身没有形成 Hybrid 下界或 D46 上界权限，后续由独立 D47 路线处理。

D47 已结果前冻结 Hybrid 加权持久化 fork 严格证书恢复合同。它显式跳过当时未获准的 D46 上界关，只读 D45 manifest `668fb0ea...`、LP 归档 `e84eb735...`、IPX/simplex_1 solution `eed2b064...` / `6f4d0276...` 与 solver execution `39b547a0...` / `8bf88766...`。56 块边界只由冻结 CSC 的 (1+nnz_j) 累积权重决定；56-worker 单 phase 先 IPX、失败才回退 simplex_1，逐块原子持久化，继续使用 D44 的 80 位向外舍入列核。源码/测试已提交并通过 OpenBayes 同哈希 Gate A：D47 `37 passed`、D40–D47 `168 passed`、全包 `622 passed`，零失败/零跳过，Gate A manifest `80591c66...`。唯一正式 IPX phase 随后在 `539.050 s` 内完成 56/56 块，80 位下界为 `232011577.83593156905560264049764989154935073609620115224488377919660126326832988 CNY`；`simplex_1` 未启动。总 manifest/execution/result/certificate SHA-256 为 `8b74c404...` / `ed978c36...` / `f8c33ca2...` / `1caa1b6...`；峰值聚合 RSS `18.838 GiB`，资源门通过，非法端点与活动残留进程均为 0。严格包络上端点 `3391819174.0195... CNY` 不是原 MILP 可行上界。D47 终态 `hybrid_r0_lower_bound_recovered` 且不得重跑，只开放另立 D46 可行上界/修复合同。

D46 已在任何实现或正式求解前冻结。它不读取 D47 包络上端点，而是在锁定的三架构原 MILP 上固定工程上界容量，先解同服务 R0 LP 生成结构化 seed，再恢复原始二元并只寻找第一个完整 MIP incumbent；随后以 D41 名称清单固定全部二元，执行最大容量 Repair A 和可选容量收缩 Repair B。只有 HiGHS `num_primal_infeasibilities=0`、独立全约束最大违反不超过 `1e-7`、年送出平均功率等式残差不超过 `1e-8 MW`、全部服务/循环/容量/目标分项审计通过，才把 80 位向上舍入目标登记为 `audited_feasible_upper_bound_cny`。D46 明确区分工程数值可行性与有理数精确证书，且首 incumbent 不代表最优。初始实现提交 `fe3b669`；Gate A 首次正式构建暴露规模锁字典漏写两个合法非线性零字段，修复提交 `4a18f42` 只补齐锁对象形状并保留全部结果前合同。OpenBayes 最终 D46/兼容/全包为 `22/204/644 passed`、零跳过，三架构 8784 h build-only 均与 D40 规模一致且未调用求解器；Gate A manifest/execution SHA-256 为 `098fc8be...` / `2ca2f3cd...`。唯一正式总批次随后运行 `8820.162 s`，总 manifest SHA-256 为 `8693722ad362b2f604f08b3ebd2bfa2c45f085e42c2ece6cf334c097db80afa9`。BESS/TES/Hybrid 的 R0 guide 分别为 `1,157,063,561.813816`、`386,559,421.67063665`、`1,186,678,269.235802 CNY`，但三者都只是连续引导。BESS 新 seed 与限定 D41 回退均未产生 incumbent；TES/Hybrid seed 分别有 `48,801/48,791` 条行不可行，固定离散 LP 判 infeasible，原 MILP 各跑满 `3600 s` 后 Primal bound 仍为 `inf`。三架构均为 `no_candidate_incumbent`，均未进入 Repair A/B；成功上界数为 0，正式可行容量、gap、项目 TAC 与技术排序均未形成。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43、D45、D47、D46、D48-R1、D49、D50 与 D52 均不得原样重跑；D44 已恢复 TES R0/R1 合法下界，D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。D46/D48-R1/D49/D50/D52 上界恢复数均为 0；D52 的阶段 0/1 checkpoint 无上界资格，阶段 2 attempt 硬墙钟只形成 `no_primal_status_closure`，不证明原 BESS MILP 不可行，TES/Hybrid 继续禁止；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44/D47 已使三架构下界门闭合。D46 三架构均无 incumbent；postmortem 不生成上界。D48-R1 三架构和 D49 BESS 都未闭合 primal 状态，D50 以 `block_path_no_incumbent` 关闭，D52 以 `no_primal_status_closure` 关闭；D51 只把缩短时域控制器推进到 `gate0_controller_validated`。D52 阶段 0/1 checkpoint 均不具上界资格，正式上界恢复数仍为 0。杨凌正式 E2、699 次边界扫描、gap 与技术排序继续禁止。Agentic 只承担检查点、哈希、阶段域、资源、bound 资格与固定状态转移编排，不替代物理模型或优化器。
