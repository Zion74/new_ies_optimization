...
- D43 已在任何新增代码或正式离线复算前冻结“冻结快照离线对偶证书恢复”合同。两个 D42 solution 归档都含完整 `439,018` 维 row dual，header 为 `dual_valid=true` 且 LP 指纹相同；D43 禁止调用优化器，只允许两个 clean child 并行执行未修改的 80 位拉格朗日证书器。当前尚未实现或运行 D43，仍无 TES/Hybrid 合法下界。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败和 D42 TES R0 证书失败均已登记。D43 结果前合同已经冻结；下一步只实现并提交离线快照审计/证书编排器和双端回归，通过后执行唯一一次正式离线复算。Hybrid 与全年可行上界修复继续禁止；
