...
- D48 Hamming 可行性—原成本修复：实现提交 `1090cd83...`；Gate A `1d894652...` 已通过且未调用正式优化。首次启动因输出目录不等于预注册路径而在 BESS 候选阶段行政终止；约 1186 s 内没有 solver log、candidate、result、上界、容量或 infeasible 状态。D48-R1 已在替代结果前冻结仅纠正路径的合同，正确路径替代批次尚待启动。
...
3. D39 代表期定量保真失败、D40 单体路线失败、D41 Gate B 最弱案例失败、D42 TES R0 证书失败与 D43 离线证书超时均已登记。D43、D45、D47 与 D46 均不得原样重跑；D44 已恢复 TES R0/R1 合法下界，D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。D46 三架构均无 incumbent。D48 合同、实现和 Gate A 已闭合；错误路径启动无结果并已终止，D48-R1 只纠正路径，不能更换 seed、容量边界、权重、选项或墙钟；
...
D36/D37 已关闭原结构化代表周的数据选择、权重和分块状态边界门；D38/R1/D39 的失败证明代表期不能恢复为正式主证据。D44/D47 已使三架构下界门闭合。D46 三架构均无 incumbent；postmortem 不生成上界。D48 Gate A 已通过，但错误路径启动不具正式资格且无结果；D48-R1 正确路径替代批次完成前，杨凌正式 E2、699 次边界扫描、gap 与技术排序继续禁止。Agentic 只承担哈希、资源、bound 资格与停止规则编排，不替代物理模型或优化器。
