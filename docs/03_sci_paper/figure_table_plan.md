...
| Tab S-E0D49 | `tab:e0d49_fuel_projection_recovery` | 原始/投影/保留二元清单、燃料编码依赖、精确提升残差、候选与原成本 repair 资格 | E0/E5 | 结果前合同已冻结，尚无实现或数值。只有精确提升和 clean 原模型 repair 均通过时才填写 BESS 工程数值上界；Gate A 与 24 h toy 不进入上界列 |
