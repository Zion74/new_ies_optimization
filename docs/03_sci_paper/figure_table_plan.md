...
| Tab S-E0D50 | `tab:e0d50_block_relax_fix` | 53 个提交阶段、固定/整数前视/未来放松/燃料投影计数、首 incumbent、完整物理轨迹、精确提升和 clean repair 资格 | E0/E5 | 合同、实现提交 `04e17643...` 与 Gate A `d74891b9...` 已闭合；Gate A 只证明 53 块域/身份和流程，不进入上界、容量或 gap 列。正式运行终态完成后才追加数值；部分阶段或投影 objective 仍不得进入结论列 |
