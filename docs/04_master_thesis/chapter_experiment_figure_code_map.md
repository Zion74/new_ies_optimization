...
| 编号 | 内容 | 目的 | 代码 / 数据 | 状态 |
|---|---|---|---|---|
| T4-1 / E1 | 受控价值分解 | 解释 BESS 与 TES 的价值来源 | `_ch4_p1_milp_compare.py` 仅作原型；新 fixed-capacity 统一模型、正式输入桥、寿命核、损失/辅机结构、成本认证门与年度经济接缝已建 | Rahman BESS 三接缝已闭合；TES 正式成本、系统级 TAC 和内生容量闭合后启动 |
