...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D44/D47 已使三架构严格全年下界闭合。D46 唯一正式总批次 manifest `8693722...`，BESS/TES/Hybrid 均为 `no_candidate_incumbent`，没有正式可行容量或上界；postmortem bundle `c74a694...` 仅定位联合离散轨迹问题。D48 第 1–11 节结果前合同已冻结“原容量边界 + 完整二元等权 Hamming 搜索 + 固定二元原成本修复”，但尚未实现或运行。严格项目账户仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC 与 E2-E6 批量入口继续阻断。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40–D47 下界/上界证据链；D46 postmortem；D48 结果前合同 | D43、D45、D47、D46 均不得原样重跑。D44/D47 使三架构下界闭合；D46 上界恢复数为 0。D48 尚待实现、Gate A 和唯一正式批次，技术排序与批量扫描仍禁止 |
