...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D43 失败已登记且 D43 不得重跑。D44 已完成 Gate A 与唯一 Gate B：IPX 24/24 块证书恢复 TES R0/R1 严格下界 `254,860,566.6193158889 CNY`，正式 manifest 为 `d6fe2f34...`；simplex_1 因 `15,195` 个所需无穷端点不合格。BESS/TES 两个下界都不是可行方案或项目 TAC，不能形成技术排序。D45 Hybrid R0 原生双快照—fork 路线已通过 OpenBayes 同哈希 Gate A（D45/D40–D45/全包为 `27/131/585 passed`，零跳过；manifest `570b801c...`），正式 Hybrid 待执行；严格项目账户仍为 `0/16`，项目运行账户为 `0/4`。下一步只允许唯一一次正式 D45 Hybrid R0，正式 TAC、三架构可行上界与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 全链证据；D43 正式失败证据；D44 Gate A、fork 并行证书与正式哈希证据；D45 Hybrid R0 合同/代码/测试/Gate A | D43 不得重跑。D44 已恢复 TES R0/R1 下界；D45 已通过 OpenBayes 同哈希 Gate A，下一步只允许唯一正式 Hybrid R0。全年可行上界修复、技术排序与批量扫描仍禁止 |
