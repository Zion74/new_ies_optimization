...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、`e0d41_strict_full_year_decomposition.py`、Gate A 证据与待建 Gate B–D | D40 单体路线未通过；D41 Gate A 已证明三架构 `R0/R1` 与完整固定无二元遗漏，下一步实现 Gate B 全年严格下界与硬墙钟 |
