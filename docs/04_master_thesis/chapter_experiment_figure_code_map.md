...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 全链证据；D43 正式失败证据；D44 正式 TES 证书；D45 正式失败证据；D47 正式 Hybrid 证书；D46 合同/源码/本地测试 | D43、D45、D47 均不得重跑。D44 已恢复 TES R0/R1 下界；D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。三架构下界闭合；D46 已实现并通过本地 24 h 三架构 toy 集成，下一步为 OpenBayes 同哈希 Gate A，技术排序与批量扫描仍禁止 |
