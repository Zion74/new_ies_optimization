...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D41 失败已登记。D42 Gate A、执行器实现门和 BESS R0 build-only 复核已通过，沿用下界 `1,144,950,604.8368804 CNY`；TES/Hybrid 尚无证书。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、D42 三架构证书与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 合同、证书器、结构门、执行器、正式驱动器与 BESS 复核结果 | BESS 前置复核通过；当前只启动 TES R0，TES 无证书则停止，D41 Gate C/D 仍禁止 |
