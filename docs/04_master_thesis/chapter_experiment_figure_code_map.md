...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D44/D47 已使三架构严格全年下界闭合。D46/D48-R1/D49/D50 的成功上界恢复数均为 0；D51 已通过缩短时域控制器 Gate 0。D52 唯一正式 BESS 已启动；阶段 0/1 checkpoint 通过完整变量映射和提交审计后进入阶段 2，但均不具上界资格。严格项目账户仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC 与 E2-E6 批量入口继续阻断。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40–D52 下界/上界/恢复证据链；D46 postmortem；D51 Gate 0；D52 Gate A/正式 checkpoints | D43、D45、D47、D46、D48-R1、D49、D50 均不得原样重跑。D44/D47 使三架构下界闭合，D46/D48-R1/D49/D50 上界恢复数均为 0；D52 唯一正式 BESS 正在运行，阶段 0/1 checkpoint 均不具上界资格，技术排序与批量扫描仍禁止 |
