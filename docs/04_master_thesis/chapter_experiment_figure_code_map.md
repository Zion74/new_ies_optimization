...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-37 已闭合 BESS/TES 账本、证据门、同服务边界、数值证书、公开成本、内生容量、材料性、代表周和分块循环边界。D38/R1/D39 与 D40 失败均已登记。D41 Gate B 为 BESS 得到严格下界 `1,144,950,604.8368804 CNY`，但 TES R0 无合法下界，总判定 `no_strict_certificate`。D42 Gate A 五案例结构门已通过，TES R0/R1 指纹同一、Hybrid 0/1 分支完整；Gate B 尚未开始。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、D42 三架构证书与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、三模块与正式 Gate A/B 证据；D42 合同、原生证书器、全年结构审计器与规范结果 | D41 BESS 下界通过但 TES R0 硬墙钟失败；D42 Gate A 五案例结构门通过，当前只允许实现并提交 Gate B 正式执行器，D41 Gate C/D 仍禁止 |
