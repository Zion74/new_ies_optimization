...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、三模块与正式 Gate A/B 证据；`docs/03_sci_paper/e0_d42_native_highs_interruptible_lagrangian_bound_contract.md` | D41 BESS 下界通过，但 TES R0 硬墙钟失败，总状态 `no_strict_certificate`；D42 已固定原生矩阵指纹、IPX、basis 检查点和独立拉格朗日证书，当前只允许实现 Gate A，D41 Gate C/D 仍禁止 |
