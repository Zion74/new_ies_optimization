...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D43 失败已登记且 D43 不得重跑。D44 已恢复 TES R0/R1 严格下界 `254,860,566.6193158889 CNY`；BESS 下界保持。D45 Hybrid R0 双证书在冻结墙钟前仅完成 `20/24`、`16/24` 块，总 manifest `668fb0ea...`，状态 `no_strict_certificate`，不得重跑。D47 唯一正式 IPX 56/56 加权持久块已恢复 Hybrid R0 严格下界 `232,011,577.8359315691 CNY`，并合法覆盖 R1/原 MILP；simplex_1 未启动，总 manifest `8b74c404...`，不得重跑。D46 已结果前固定并实现最大容量锚点、R0 seed、首 incumbent、完整二元 Repair A/B 与可行性/资源门；最终源码提交 `4a18f42` 已通过 OpenBayes `22/204/644 passed` 与三架构 8784 h build-only Gate A，唯一正式运行尚未启动。严格项目账户仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、三架构可行上界与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 全链证据；D43 正式失败证据；D44 正式 TES 证书；D45 正式失败证据；D47 正式 Hybrid 证书；D46 合同/源码/Gate A | D43、D45、D47 均不得重跑。D44 已恢复 TES R0/R1 下界；D47 已恢复 Hybrid R0 下界并覆盖 R1/原 MILP。三架构下界闭合；D46 同哈希 Gate A 已通过，下一步仅允许唯一正式总批次，技术排序与批量扫描仍禁止 |
