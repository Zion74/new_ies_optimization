...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D42 失败已登记。D42 保留 BESS 下界 `1,144,950,604.8368804 CNY`，但 TES R0 没有在冻结硬墙钟内生成合法证书，D42 已停止且 Hybrid 未启动。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、新的三架构严格证书路线与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 合同、证书器、结构门、执行器、正式驱动器、BESS 复核与 TES R0 失败证据 | TES R0 为 `no_strict_certificate`，D42 已停止且 Hybrid 未启动；须另立结果前证书恢复合同，全年上界修复与技术排序仍禁止 |
