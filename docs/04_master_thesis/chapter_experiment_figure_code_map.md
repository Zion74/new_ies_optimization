...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-37 已闭合 BESS/TES 账本、证据门、同服务边界、数值证书、公开成本、内生容量、材料性、代表周和分块循环边界。原 D38 高热状态物理失败，R1 baseline 发生代表期—全年反转；D39 修复分类但弃电率误差仍为 `5.1762` 个百分点，Gate B 失败并终止 Gate C/D。D40 已通过真实 8784 h 四架构服务、结构与资源 Gate A，OpenBayes 完整回归为 `469 passed`；正式 BESS 在 `4527.395 s` 后仍无结果并被记为 `monolithic_not_viable`，全年单体路线失败。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、严格全年分解证据与 E2-E6 批量入口尚待实现。
...
| 编号 | 内容 | 目的 | 代码 / 数据 | 状态 |
|---|---|---|---|---|
| T4-1 / E1 | 受控价值分解 | 解释 BESS 与 TES 的价值来源 | `planning_model.py`、`e0d34_endogenous_capacity_sample.py`、`e0d35_tes_materiality.py`、`e0d35_materiality_bundle.py`；`_ch4_p1_milp_compare.py` 仅作旧原型 | D35–D37 结论已冻结；D38/R1/D39 失败均已登记。D40 全年计算门通过前不得恢复机制扫描或写成技术赢家 |
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同、`e0d40_full_year_compute_gate.py`、`e0d40_gate_b_solver.py`、`数据采集/e0d40_full_year_compute_gate/` 与测试；后续严格分解模块 | D40 Gate A 已通过，但正式 BESS 为 `monolithic_not_viable`，故全年单体路线未通过；下一步建立带父进程硬墙钟和严格全年界的分解合同 |
