...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D43 失败已登记且 D43 不得重跑。D44 已恢复 TES R0/R1 严格下界 `254,860,566.6193158889 CNY`；BESS 下界保持，二者都不是可行方案或项目 TAC。D45 Hybrid R0 双证书在冻结墙钟前仅完成 `20/24`、`16/24` 块，总 manifest `668fb0ea...`，状态 `no_strict_certificate`，不得重跑。D47 的 56 加权持久块、IPX 优先实现与 OpenBayes Gate A 已通过（`37/168/622 passed`），唯一正式证书尚未启动，D46 上界仍不获准。严格项目账户仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、三架构可行上界与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 全链证据；D43 正式失败证据；D44 正式 TES 证书；D45 正式失败证据；D47 加权持久化源码/测试与 Gate A | D43、D45 均不得重跑。D44 已恢复 TES R0/R1 下界；D45 为 `no_strict_certificate`。D47 已完成 Gate A，下一步只允许唯一正式证书；全年上界修复、技术排序与批量扫描仍禁止 |
