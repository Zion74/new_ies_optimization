...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-37 已闭合 BESS/TES 账本、证据门、同服务边界、数值证书、公开成本、内生容量、材料性、代表周和分块循环边界。D38 执行器、bundle 审计和周级诊断已实现；原高热紧 PCC 状态物理不可行且非漏周，R1 baseline 又发生代表期可行、真实 8784 h 不可行的反转。D39 Gate A 已生成并复现服务感知八周数据，双端完整回归为 `452 passed`；Gate B 尚待执行。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、新的有效全年预验证与 E2-E6 批量入口尚待实现。
