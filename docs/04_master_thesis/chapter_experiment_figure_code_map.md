...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同、`e0d40_full_year_compute_gate.py`、`数据采集/e0d40_full_year_compute_gate/` 与测试；后续全年求解/严格分解模块 | D40 四架构 Gate A 已通过；只有后续三种储能 Gate B 均达 `0.1%` 或严格不可行，才能进入局部全年网格 |
