...
除明确写出完整路径外，本文件中的 `model.py`、`scenarios.py`、`components/...` 等路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-13 已建立唯一 Rahman BESS 正式来源候选及主要非电芯账本；本地 263 项通过，OpenBayes 最近 258 项通过。杨凌现场泵参数、BESS 三接缝、TES 正式参数、完整 TAC、容量规划与 E1-E6 批量入口尚待实现。
...
| T2-5 | HiGHS 求解验证 | 验证可行性、MIP gap、复现和资源占用 | `solver.py`、`heat_bridge.py` + `highspy` | 当前代码基线：本地 `263 passed in 34.57s`，OpenBayes 最近 `258 passed in 21.36s`；E0-D-13 尚未同步远端 |
...
| 编号 | 内容 | 目的 | 代码 / 数据 | 状态 |
|---|---|---|---|---|
| T4-1 / E1 | 受控价值分解 | 解释 BESS 与 TES 的价值来源 | `_ch4_p1_milp_compare.py` 仅作原型；新 fixed-capacity 统一模型、正式输入桥、寿命核、损失/辅机结构、成本认证门与年度经济接缝已建 | Rahman BESS 来源已合格；BESS 三接缝、TES 正式成本、完整 TAC 和内生容量闭合后启动 |
