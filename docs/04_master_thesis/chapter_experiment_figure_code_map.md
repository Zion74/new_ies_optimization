...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-37 已闭合 BESS/TES 账本、证据门、同服务边界、数值证书、公开成本、内生容量、材料性、代表周和分块循环边界。D38/R1/D39 与 D40 失败均已登记。D41 Gate A 通过；Gate B 为 BESS 得到严格下界 `1,144,950,604.8368804 CNY`，但 TES R0 在 `720.462 s` 硬墙钟内无合法下界，TES R1、Hybrid 和 Gate C/D 均按规则停止，总判定 `no_strict_certificate`。D42 原生 HiGHS 可中断拉格朗日下界合同已结果前冻结，Gate A 核心实现和双端 501 项回归已完成；真实全年 build-only 结构门和 Gate B 尚未开始。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、D42 三架构证书与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、三模块与正式 Gate A/B 证据；D42 合同、`src/tes_bess_boundary/e0d42_native_highs_certificate.py` 与测试 | D41 BESS 下界通过，但 TES R0 硬墙钟失败，总状态 `no_strict_certificate`；D42 已固定原生矩阵指纹、IPX、basis 检查点和独立拉格朗日证书，核心实现通过，当前只允许执行真实全年 build-only 结构门，D41 Gate C/D 仍禁止 |
