...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。D38–D42 失败已登记。D42 保留 BESS 下界 `1,144,950,604.8368804 CNY`，但 TES R0 无合法证书，Hybrid 未启动。D43 结果前合同已冻结，只读复算两个完整 D42 dual 快照且不调用优化器；尚未实现或运行。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、新的三架构严格证书路线与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40/D41 合同与失败证据；D42 全链证据；D43 冻结快照离线证书合同 | D43 第 1–10 节已结果前冻结，尚未实现或运行；只允许先提交只读编排器，Hybrid、全年上界修复与技术排序仍禁止 |
