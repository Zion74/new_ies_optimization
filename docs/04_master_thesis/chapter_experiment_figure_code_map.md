...
除明确写出完整路径外，本文件中的 `model.py`、`scenarios.py`、`components/...` 等路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-17 已闭合 BESS fixed-capacity 账本、TES 12 账户门禁、全系统 EAC 上限内核和实际年度结果适配；E0-D-18 新增 24 h 精确点与 336 h 有界性能产物。完整回归本地/远端均 288 项通过；杨凌现场泵参数、TES 正式参数、同范围系统成本、正式 TAC、容量规划与 E1-E6 批量入口尚待实现。
...
| T2-4 | 双品位 TES 单元验证 | 验证 HT/MT、端口、盐量、品位方向与分部件寿命 | `components/molten_salt.py`、`tes_loss_auxiliary.py`、`model.py`、`formal_tes_costs.py`、`tes_break_even.py`、`tes_break_even_adapter.py` | 三温区/五端口、损失/辅机、成本门、实际结果适配和路径紧 Big-M 通过；12 账户仍阻断，EAC 区间只能探索使用 |
| T2-5 | HiGHS 求解验证 | 验证可行性、MIP gap、复现和资源占用 | `solver.py`、`heat_bridge.py`、`e0d17_exploration.py`、`e0d18_performance.py` + `highspy` | 本地 `288 passed in 56.38s`，OpenBayes `288 passed in 21.23s`；24 h gap 0，336 h 主 gap 0.004800，canonical 跨平台一致 |
...
| 编号 | 内容 | 目的 | 代码 / 数据 | 状态 |
|---|---|---|---|---|
| T4-1 / E1 | 受控价值分解 | 解释 BESS 与 TES 的价值来源 | `_ch4_p1_milp_compare.py` 仅作原型；E0-D-18 已补 24 h 精确点和 336 h 有界筛查 | 两窗口筛查不等于 E1；TES 正式成本、同范围系统成本、正式 TAC 和内生容量闭合后才启动 E1 |
