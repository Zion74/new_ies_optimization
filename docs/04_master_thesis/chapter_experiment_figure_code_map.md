...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同、`e0d40_full_year_compute_gate.py`、`e0d40_gate_b_solver.py`、`数据采集/e0d40_full_year_compute_gate/` 与测试；后续严格分解模块 | D40 四架构 Gate A 已通过，Gate B 监控/审计接入器已本地实现；只有三种储能正式 Gate B 均达 `0.1%` 或严格不可行，才能进入局部全年网格 |
