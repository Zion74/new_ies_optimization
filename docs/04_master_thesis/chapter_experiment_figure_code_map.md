...
除明确写出完整路径外，本文件中的代码路径均相对于 `风光火+熔盐储热/tes_bess_boundary/`。E0-D-14–D-37 已闭合 BESS/TES 账本、证据门、同服务边界、数值证书、公开成本、内生容量、材料性、代表周和分块循环边界。D38/R1/D39 与 D40 失败均已登记。D41 BESS 下界为 `1,144,950,604.8368804 CNY`，TES R0 无合法下界。D42 Gate A 与 Gate B 执行器实现门已通过，双端全包 `519 passed`；正式下界尚未开始。该下界不是可行方案或项目 TAC，不能形成技术排序。严格证据路线仍为 `0/16`，项目运行账户为 `0/4`；正式 TAC、D42 三架构证书与 E2-E6 批量入口尚待完成。
...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、三模块与正式 Gate A/B 证据；D42 合同、证书器、结构门、父进程执行器与正式驱动器 | D41 TES 下界失败；D42 Gate A 与执行器实现门通过，当前只允许 BESS build-only 复核，通过后启动 TES，D41 Gate C/D 仍禁止 |
