...
| T4-5 / E5 | 8784 h 全年证据与时间方法 | 建立不依赖失真代表期的正式边界路线 | D36–D39 失败证据；D40 合同/代码/证据；D41 合同、`e0d41_strict_full_year_decomposition.py`、`e0d41_gate_b_lower_bound.py`、`e0d41_gate_b_bundle.py` 与正式 Gate A/B 证据 | D41 BESS 下界通过，但 TES R0 硬墙钟失败，总状态 `no_strict_certificate`；Gate C/D 禁止，下一步另立 D42 处理全年 LP 收敛与合法 dual 提取 |
