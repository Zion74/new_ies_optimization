...
- **D40 全年优先计算门**：已在任何 D40 构造或求解结果产生前冻结真实 8784 h 单块、baseline 同服务、三种储能架构、HiGHS `0.1%` 正式 gap、3600 s/案例和内存停止规则。代表期不再进入正式容量规划。OpenBayes Gate A 已通过：四架构均为线性单全年循环，Hybrid 含 `685,194` 个活动变量、`96,625` 个二元变量和 `667,662` 条活动约束；峰值 RSS `0.645 GiB`、构造后可用内存 `96.705 GiB`，总 manifest SHA-256 为 `23e0831ed017ca794a73b897196495079db3ace847fe840d51c1fa60af0de577`。Gate B 尚未启动；`0.1%–0.5%` 只记为有界未合格，超过 `0.5%` 或资源超限则转向严格分解合同；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通；E0-D-23 双窗口、D24 证据路线、D25 项目取证合同、D26–D32 数值证书与筛查，以及 D33–D40 Gate A 均在远端执行。当前服务器完整回归为 `462 passed in 33.82s`；Windows 最近完整回归为修订前 `460 passed`，修订后 D40 定向回归 `8 passed`。正式求解仅使用 HiGHS；D40 首轮审计误拒绝产物已隔离，不进入正式 Gate A 证据；
