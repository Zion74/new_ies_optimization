...
- **D49 物理优先燃料投影恢复（OpenBayes Gate A 已通过）**：原 `79,057` 个 BESS 二元变量预注册为 `52,704` 个 CHP 燃料相邻段编码位和 `26,353` 个保留物理位；候选只对物理位做等权 Hamming，随后逐时精确回填燃料段并在 clean 原始成本模型中修复。核心/执行器/测试提交 `86a8b80e...`；OpenBayes 同哈希 D49、D40–D49、全包为 `14/219/673 passed`，零失败/零跳过。BESS/TES/Hybrid 8784 h build-only 均通过分区、依赖、原规模/容量边界、约束身份和全部燃料段/结点提升证明，保留物理二元分别为 `26,353/35,136/43,921`，三案均 `solver_invoked=false`。Gate A manifest `11b283d6...` 只授权一次 BESS 正式方法门，不产生可行容量或上界；失败转 D50 而不延长 D49 墙钟；
