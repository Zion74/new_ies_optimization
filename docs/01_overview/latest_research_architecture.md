...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D43、D45、D47、D46 均不得原样重跑。D48 首次启动因输出目录不等于预注册路径而在 BESS 候选阶段行政终止；约 1186 s 内无 solver log、candidate、result 或数值结论，残留进程为 0。D48-R1 已在替代结果前冻结仅纠正路径的合同；替代启动前，正式 E2–E4、gap 收缩、项目 TAC 和技术赢家继续阻断；
