...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D43、D45、D47、D46 均不得原样重跑。D48 首次错误路径启动已作为无结果行政失败归档；D48-R1 正确路径替代批次正在运行。BESS/TES 候选阶段分别在 `3720.176/3720.581 s` 父级硬墙钟受控终止，均未生成 candidate、repair、容量或上界，活动残留均为 0；两者的合同状态都只能登记为 `no_primal_status_closure`，不构成相应架构不可行。编排器未重启前两案，已按冻结顺序进入 Hybrid；总 `formal_manifest.json` 尚未生成。正式 E2–E4、gap 收缩、项目 TAC 和技术赢家继续阻断；
