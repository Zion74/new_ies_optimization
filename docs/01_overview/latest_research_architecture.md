...
- **D33 公开成本与容量核**：已建立聚合储热包/分项台账两套互斥 TES 公开敏感性组合，强制 12 账户各覆盖一次、作者价格年和相似部件代理显式确认，并保持 `formal_project_eligible=false`；BESS/TES 线性内生容量核已通过 HiGHS 小模型测试。下一步是接入双机 CHP/PCC、损失辅机、两个独立额定放能测试和 BESS 正式账本，不再优先追加 D31/D32 同类 336 h 数值紧化；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通；E0-D-23 双窗口、D24 证据路线、D25 项目取证合同、D26–D32 数值证书与筛查，以及 D33 公开成本/容量核的 19 项定向和 396 项全回归均在远端执行，正式求解仅使用 HiGHS；
