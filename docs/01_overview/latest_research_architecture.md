...
- **D52 全年检查点化有界回退（实现已完成，Gate A 待执行）**：结果前合同只开放 BESS，逐字节沿用 D50 的真实 `8784 h` 模型与服务，保留 `336 h` 整数前视、`168 h` 提交和 48 h 年尾，共 `53` 个阶段；使用常数零可行性目标、一步回退、每阶段至多 3 个不同模式、全批至多 8 次回退和至多 69 次 solver 尝试，再执行确定性燃料提升与 clean 原成本 repair。独立核心、BESS-only 监控执行器和测试已实现；本地 D52、D51–D52、D40–D52、全包分别为 `17/32/261/715 passed`，后两组及全包另有 `5` 个 Linux-only 跳过。当前尚未完成 OpenBayes 零跳过 Gate A，也未创建正式目录，`formal_run_permitted=false`，年度 optimize 仍禁止；
