...
- **D43 冻结快照离线证书恢复（结果前合同已冻结，尚未实现）**：两个 D42 solution 归档均包含完整 `439,018` 维 row dual，`dual_valid=true` 且 LP 指纹一致。D43 禁止重跑求解器，只允许两个 clean child 并行把冻结 dual 送入未修改的 80 位证书器，并预注册“合法证书取 Decimal 最大值”的规则；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。TES 准备/IPX/simplex 的峰值进程树 RSS 分别为 `2.167/0.706/0.914 GiB`，最低可用内存均超过 `95 GiB`，失败不是内存耗尽。下一步先完成并提交 D43 只读编排器与双端回归，再执行唯一一次离线证书恢复；Hybrid、技术排序和全年上界修复继续阻断；
