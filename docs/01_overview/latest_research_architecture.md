...
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通；E0-D-23 双窗口、D24 证据路线、D25 项目取证合同、D26–D32 数值证书与筛查，以及 D33–D39 Gate A 定向、全回归和预验证均在远端执行；当前本地/服务器完整回归均为 `452 passed`，正式求解仅使用 HiGHS；首轮混合代码哈希产物已隔离，不进入证据；
