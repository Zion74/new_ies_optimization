...
- **D49 物理优先燃料投影恢复（已实现，OpenBayes Gate A 未执行）**：原 `79,057` 个二元变量预注册为 `52,704` 个 CHP 燃料相邻段编码位和 `26,353` 个保留物理位；候选只对物理位做等权 Hamming，随后逐时精确回填燃料段并在 clean 原始成本模型中修复。核心/执行器/测试提交 `86a8b80e...`，本地 D49 定向 `14 passed`，三架构 24 h 候选—提升—原成本修复均通过；本地兼容/全包仅有既有 D42 的 Windows 中文路径 basis 写盘 3 失败。正式准入仍要求 OpenBayes 同哈希 D49 定向、D40–D49 与全包零失败/零跳过及三架构 8784 h build-only；首次正式门只授权 BESS，失败转 D50 而不延长 D49 墙钟；
