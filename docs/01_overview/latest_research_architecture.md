...
- **D40 全年优先计算门**：已在任何 D40 构造或求解结果产生前冻结真实 8784 h 单块、baseline 同服务、三种储能架构、HiGHS `0.1%` 正式 gap、3600 s/案例和内存停止规则。代表期不再进入正式容量规划；Gate A 服务/结构/资源审计器及 6 项测试已实现，但尚未在服务器构造四个全年模型。Gate B 只在 Gate A 通过后求解 BESS/TES/Hybrid；`0.1%–0.5%` 只记为有界未合格，超过 `0.5%` 或资源超限则转向严格分解合同；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通；E0-D-23 双窗口、D24 证据路线、D25 项目取证合同、D26–D32 数值证书与筛查，以及 D33–D39 Gate B 均在远端执行；当前本地完整回归为 `460 passed`，服务器同步前仍为 `454 passed`，正式求解仅使用 HiGHS；首轮混合代码哈希产物已隔离，不进入证据；
