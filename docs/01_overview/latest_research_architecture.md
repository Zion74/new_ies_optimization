...
- **D42 原生 HiGHS 可中断拉格朗日下界（Gate A 通过）**：保持 D40/D41 的 8784 h 模型、服务与成本口径不变，先用显式预求解矩阵和 IPX 争取快速最优，再用可恢复 dual simplex 检查点提高下界。Gate A 五案已证明 TES R0/R1 原始与 presolve 指纹一致，Hybrid R1 的 `bess.installed=0/1` 两支完整且均为纯 LP；汇总 manifest SHA-256 为 `2d049208e8d8bafffce6a69878555d4d478bb305f8e5c2de42743c69cc9831d1`。正式资格仍只能来自不少于 80 位定向舍入证书；PDLP、代表期、缩放搜索与延长 D41 墙钟排除。Gate B 尚未启动；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通，正式求解仅使用 HiGHS。当前同哈希完整回归为 `508 passed in 34.22s`，Windows 完整回归为 `508 passed in 66.17s`；Gate A 五案峰值 RSS `2.288–2.407 GiB`。下一步只允许实现、测试并独立提交 D42 Gate B 父进程执行器；提交后才允许正式 IPX/simplex；
