...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。D45 唯一正式运行已结束且不得重跑；`d46_feasible_upper_bound_contract_permitted=false`，下一步只能另立并在结果前冻结缩放、对偶修复、R1 分支或严格分解型 Hybrid 下界恢复合同。正式 E2–E4、全年可行上界修复和技术赢家继续阻断；
