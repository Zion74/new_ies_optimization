...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D47 结束后活动残留进程为 0，唯一正式机会已用尽且不得重跑。BESS、TES、Hybrid 现均具有至少一个严格全年下界，但这些下界都不是可行上界、容量或项目 TAC，不能据此技术排序。D47 已使 `d46_feasible_upper_bound_contract_permitted=true`，下一步只允许另立并冻结 D46 可行上界/修复合同；正式 E2–E4、容量恢复和技术赢家仍阻断；
