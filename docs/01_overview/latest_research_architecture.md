...
- **D46 三架构可行上界合同（结果前已冻结）**：D47 已使 `d46_feasible_upper_bound_contract_permitted=true`。D46 固定使用工程上界容量锚点、R0 连续引导、原 MILP 首个 incumbent、完整二元固定和全年 LP Repair A/B，为 BESS/TES/Hybrid 分别寻找第一组审计可行上界；原模型、8784 h 服务、容量边界、seed/锚点、`1e-8/1e-7` 残差门、12-thread HiGHS、阶段/总墙钟和资源停止规则均已结果前固定。源码/测试已提交 `fe3b669`，本地 D46 `22 passed`、D40–D47/规划 `199 passed + 5 skipped`、全包 `639 passed + 5 skipped`；24 h 三架构只形成 Gate A toy upper bound，OpenBayes Gate A 与正式 D46 均未启动；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D47 结束后活动残留进程为 0，唯一正式机会已用尽且不得重跑。BESS、TES、Hybrid 现均具有至少一个严格全年下界，但这些下界都不是可行上界、容量或项目 TAC。下一步只允许对 `fe3b669` 执行同哈希 OpenBayes Gate A，通过后才能启动唯一正式 D46 总批次；正式 E2–E4、gap 收缩、项目 TAC 和技术赢家仍阻断；
