...
- **D41 严格全年界—修复分解（Gate A 通过，Gate B 执行器已冻结）**：已在任何 D41 代码、模型或数值产生前冻结 `R0` 全连续松弛、`R1` 拓扑整数松弛、168 h 提交/336 h 前视的候选轨迹和原始 8784 h 固定离散轨迹可行修复。正式下界只能来自可行域包含原 MILP 的全年松弛 dual，正式上界只能来自保留全部真实小时、年度服务和单全年循环的原模型可行解；候选分块永久标记 `candidate_only`。Gate A 已在 OpenBayes 通过：BESS/TES/Hybrid 的原始二元为 `79,057/87,840/96,625`，R0 均剩 0，R1 只剩 `1/0/1` 个时间不变拓扑二元，完整固定后均无漏网二元；汇总 manifest SHA-256 为 `50240e7ae557afa5633b29904585f1c1297a527343e467ce76d7766ce0177937`。Gate B 执行器已在本地完成独立 `720/1320 s` 硬墙钟、5 s 心跳、资源门、finite dual 审计与 R1 候选轨迹导出，完整回归 `486 passed`；正式 8784 h 求解尚未启动，仍无下界、上界、容量、成本或 gap；下一步先服务器回归，再按 BESS→TES→Hybrid 执行 Gate B；
