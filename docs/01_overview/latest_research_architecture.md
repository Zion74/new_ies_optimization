...
- **D50 全年耦合物理分块 Relax-and-Fix（结果前方法已冻结）**：只允许 BESS；始终保留同一个 8784 h 年度 PCC、弃电帽、吞吐和单全年循环模型，将 `52,704` 个燃料编码位连续化，只在连续 `336 h` 前视带内开放物理二元并每次固定前 `168 h`，连续容量保持原边界内自由。53 个阶段全部完成后才允许执行 D49 的确定性燃料提升与 clean 原成本固定二元 LP。当前尚未实现、尚未执行 Gate A 或正式求解，不产生容量、上界或 gap；
