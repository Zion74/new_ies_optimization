...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS；D44 本身没有调用优化器或原生求解器。D44 成功只关闭 TES 下界缺口，不给出可行上界、容量、项目 TAC、gap 或技术排序。下一步仅允许先冻结 Hybrid 下界结果前合同；正式 E2–E4、全年可行上界修复和技术赢家继续阻断；
