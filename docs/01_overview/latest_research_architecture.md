...
- **D41 严格全年界—修复分解（Gate A 通过，Gate B 接入修复待服务器回归）**：已在任何 D41 代码、模型或数值产生前冻结 `R0` 全连续松弛、`R1` 拓扑整数松弛、168 h 提交/336 h 前视的候选轨迹和原始 8784 h 固定离散轨迹可行修复。正式下界只能来自可行域包含原 MILP 的全年松弛 dual，正式上界只能来自保留全部真实小时、年度服务和单全年循环的原模型可行解；候选分块永久标记 `candidate_only`。Gate A 已在 OpenBayes 通过：BESS/TES/Hybrid 的原始二元为 `79,057/87,840/96,625`，R0 均剩 0，R1 只剩 `1/0/1` 个时间不变拓扑二元，完整固定后均无漏网二元；汇总 manifest SHA-256 为 `50240e7ae557afa5633b29904585f1c1297a527343e467ce76d7766ce0177937`。Gate B 初次 BESS 接入因误用 `E0CTimeSeries.periods` 在调用 HiGHS 前拒绝，且暴露错误继续 R1 的编排缺陷；失败产物已隔离。修复改用 `period_count` 并在无合法下界时停止后续阶段，本地完整回归 `488 passed`；仍无下界、上界、容量、成本或 gap；下一步先服务器同哈希回归，再重新启动 BESS；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通；E0-D-23 双窗口、D24 证据路线、D25 项目取证合同、D26–D32 数值证书与筛查，以及 D33–D41 Gate A 均在远端执行。当前服务器完整回归为 `478 passed in 34.20s`；Windows D40/D41 定向回归为 `24 passed in 3.02s`。正式求解仅使用 HiGHS；D40 首轮审计误拒绝产物已隔离，不进入正式 Gate A 证据；
