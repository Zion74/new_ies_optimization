...
- **D47 Hybrid 加权持久化证书（结果前合同已冻结）**：D46 继续保留为成功后才可能开放的上界关，本轮跳至 D47 新严格分解合同。D47 只读 D45 同哈希 LP 与双 row dual，不调用 HiGHS `run()`；按每列固定开销加非零元数生成 56 个确定性连续块，以 56 个 fork worker 先认证 IPX、失败才回退 simplex_1，并逐块原子落盘。原 24 等列块非零元负载比 `2.717`，56 加权块工作量比约 `1.054`；源码、Gate A 与正式结果尚未实现；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。D45 不得重跑，`d46_feasible_upper_bound_contract_permitted=false`；下一步只允许实现 D47 源码/测试并完成独立提交与 OpenBayes 同哈希 Gate A，正式 D47 必须在此后另行准入。正式 E2–E4、全年可行上界修复和技术赢家继续阻断；
