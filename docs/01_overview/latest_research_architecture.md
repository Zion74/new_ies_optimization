...
- **D47 Hybrid 加权持久化证书（Gate A 已通过）**：D46 继续保留为成功后才可能开放的上界关，本轮跳至 D47 新严格分解合同。D47 只读 D45 同哈希 LP 与双 row dual，不调用 HiGHS `run()`；按每列固定开销加非零元数生成 56 个确定性连续块，以 56 个 fork worker 先认证 IPX、失败才回退 simplex_1，并逐块原子落盘。源码/测试已由提交 `1515eca` 固定；OpenBayes D47、D40–D47 和全包为 `37/168/622 passed`，零失败、零跳过。正式 LP 的 56 块总工作权重 `2,525,502`，块权重比 `1.054486`；正式结果尚未运行；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，正式证书计算只读使用 HiGHS LP 结构，不调用求解器。D45 不得重跑，D47 Gate A manifest 为 `80591c66...`，已开放唯一一次正式 D47；Hybrid 下界在正式证书完成前仍不存在，D46 可行上界、正式 E2–E4 与技术赢家继续阻断；
