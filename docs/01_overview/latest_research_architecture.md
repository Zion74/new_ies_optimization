...
- **D52 全年检查点化有界回退（唯一正式 BESS 运行中）**：结果前合同只开放 BESS，逐字节沿用 D50 的真实 `8784 h` 模型与服务，保留 `336 h` 整数前视、`168 h` 提交和 48 h 年尾，共 `53` 个阶段；使用常数零可行性目标、一步回退、每阶段至多 3 个不同模式、全批至多 8 次回退和至多 69 次 solver 尝试，再执行确定性燃料提升与 clean 原成本 repair。实现提交 `c3b2e0cf...` 的 OpenBayes 同哈希 Gate A 以 D52/D40–D52/全包 `17/266/720 passed`、零跳过闭合；全年 build-only 命中 `597,318` 个活动变量、`79,057` 个二元变量、`527,053` 条活动约束和零非线性。唯一正式 BESS 已按冻结参数启动；阶段 0/1 均以 HiGHS `Optimal` 捕获完整 `597,318` 变量映射，分别提交 `505/504` 个物理二元，累计固定 `1,009` 个，最大整数残差分别为 `1.7542e-14/1.5543e-15`，checkpoint SHA-256 为 `9af8c1c5...` / `c8328947...`，随后进入阶段 2。两个 checkpoint 均明确 `formal_upper_bound_eligible=false`；在 53 阶段、精确提升和 clean repair 全部闭合前，仍没有正式 candidate、容量、上界或 gap，TES/Hybrid 继续禁止；
