...
- **D50 全年耦合物理分块 Relax-and-Fix（实现与 Gate A 已闭合）**：只允许 BESS；始终保留同一个 8784 h 年度 PCC、弃电帽、吞吐和单全年循环模型，将 `52,704` 个燃料编码位连续化，只在连续 `336 h` 前视带内开放物理二元并每次固定前 `168 h`，连续容量保持原边界内自由。提交 `04e17643...` 已实现单 clean child / 单全年模型的 53 阶段路径；OpenBayes Gate A `d74891b9...` 为 `15/244/688 passed`、零跳过且 `formal_optimization_invoked=false`。当前只开放唯一正式 BESS 流水线，尚无正式候选、容量、上界或 gap；
