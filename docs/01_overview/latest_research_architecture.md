...
- **D46 三架构可行上界合同与 Gate A（已通过）**：D47 已使 `d46_feasible_upper_bound_contract_permitted=true`。D46 固定使用工程上界容量锚点、R0 连续引导、原 MILP 首个 incumbent、完整二元固定和全年 LP Repair A/B，为 BESS/TES/Hybrid 分别寻找第一组审计可行上界；原模型、8784 h 服务、容量边界、seed/锚点、`1e-8/1e-7` 残差门、12-thread HiGHS、阶段/总墙钟和资源停止规则均已结果前固定。最终源码提交 `4a18f42` 已通过 OpenBayes 同哈希 Gate A：D46/兼容/全包为 `22/204/644 passed`，零跳过；三架构 8784 h build-only 规模与 D40 完全一致、非线性为 0，求解器未调用；Gate A manifest `098fc8be...`；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D47 结束后活动残留进程为 0，唯一正式机会已用尽且不得重跑。BESS、TES、Hybrid 现均具有至少一个严格全年下界，但这些下界都不是可行上界、容量或项目 TAC。D46 Gate A 已仅开放唯一正式 BESS→TES→Hybrid 总批次；该批次完成前，正式 E2–E4、gap 收缩、项目 TAC 和技术赢家仍阻断；
