...
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D43、D45、D47、D46 均不得原样重跑。D48 首次错误路径启动已作为无结果行政失败归档；D48-R1 正确路径替代批次已经启动。BESS 候选阶段在 `3720.176 s` 父级硬墙钟受控终止，未生成 candidate、repair、容量或上界，活动残留为 0；其合同状态为 `no_primal_status_closure`，不构成 BESS 不可行。TES 候选阶段正按冻结顺序运行，总 `formal_manifest.json` 尚未生成。正式 E2–E4、gap 收缩、项目 TAC 和技术赢家继续阻断；
