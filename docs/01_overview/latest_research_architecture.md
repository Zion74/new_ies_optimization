...
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。TES 准备/IPX/simplex 的峰值进程树 RSS 分别为 `2.167/0.706/0.914 GiB`，最低可用内存均超过 `95 GiB`，失败不是内存耗尽。D42 已关闭；下一步只能先另立结果前的证书恢复合同，不能启动技术排序或全年上界修复；
