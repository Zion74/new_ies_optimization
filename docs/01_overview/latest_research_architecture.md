...
- **D45 Hybrid R0 严格下界（合同与实现已完成）**：只求 Hybrid 的全连续 R0 松弛，不把 R1 两个拓扑分支设为成功条件。正式 prepare 必须复现原始 `667,662 × 685,194` 与 presolved `495,630 × 539,546` LP 指纹；IPX/simplex_1 并行生成 row dual，随后用 D44 的 24 块/快照、80 位 fork 核认证。R0 下界按可行域包含关系覆盖 R1 和原 MILP；Windows 候选测试 `25 passed + 2 Linux-only skipped`，尚未运行正式 Hybrid；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。D45 必须先完成独立提交与 Linux 同哈希 Gate A，才允许唯一一次正式 Hybrid R0；正式 E2–E4、全年可行上界修复和技术赢家继续阻断；
