...
- **D51 检查点化有界回退 Gate 0（结果前合同已冻结）**：只授权缩短时域实现与验证逐尝试原子检查点、clean 重放、一步回退、每阶段最多三次尝试及 no-good cut；候选阶段测试常数零可行性目标，最终上界仍只能来自原成本 clean repair。当前 `formal_8784h_optimization_invoked=false`，尚未开放任何全年正式运行、容量、上界或技术结论；
