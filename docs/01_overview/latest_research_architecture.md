...
- **D50 全年耦合物理分块 Relax-and-Fix（唯一正式 BESS 运行中）**：只允许 BESS；始终保留同一个 8784 h 年度 PCC、弃电帽、吞吐和单全年循环模型，将 `52,704` 个燃料编码位连续化，只在连续 `336 h` 前视带内开放物理二元并每次固定前 `168 h`，连续容量保持原边界内自由。提交 `04e17643...` 与 Gate A `d74891b9...` 已闭合。`2026-07-17T05:59:38Z` 检查点为阶段 `0/1` 已提交、阶段 `2` 运行中，累计固定 `1,009` 个物理二元，资源门正常；阶段产物均不具正式上界资格，尚无正式候选、容量、上界或 gap；
