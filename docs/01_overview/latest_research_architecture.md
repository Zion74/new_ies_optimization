...
- **D42 原生 HiGHS 可中断拉格朗日下界（Gate A 与执行器实现门通过）**：保持 D40/D41 的 8784 h 模型、服务与成本口径不变。Gate A 五案已证明 TES R0/R1 原始与 presolve 指纹一致，Hybrid R1 的 `bess.installed=0/1` 两支完整且均为纯 LP；manifest SHA-256 为 `2d049208e8d8bafffce6a69878555d4d478bb305f8e5c2de42743c69cc9831d1`。父进程硬墙钟、IPX→四段 simplex、同指纹 basis、80 位证书、哈希链与 BESS→TES→Hybrid 准入驱动器也已双端回归通过。PDLP、代表期、缩放搜索与延长 D41 墙钟排除；正式 Gate B 数值尚未产生；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存已连通，正式求解仅使用 HiGHS。当前同哈希完整回归为 OpenBayes `519 passed in 34.55s`、Windows `519 passed in 92.69s`；Gate A 五案峰值 RSS `2.288–2.407 GiB`。下一步只执行 BESS R0 build-only 与 D41 下界复核，通过后才允许 TES R0 正式 IPX/simplex；
