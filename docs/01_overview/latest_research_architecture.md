...
- **D43 冻结快照离线证书恢复（本地实现门通过）**：两个 D42 solution 归档均包含完整 `439,018` 维 row dual，`dual_valid=true` 且 LP 指纹一致。D43 只读编排器已实现 D42 元数据/快照双哈希链、数组/finite dual 准入、两个 clean child、资源停止、Decimal 最大合法下界选择和只读 README；本地 15 项新增、D40–D43 80 项和全包 534 项回归通过，尚未读取正式 dual；
- **服务器**：OpenBayes 60 核 / 约 100 GB 内存，正式求解仅使用 HiGHS。TES 准备/IPX/simplex 的峰值进程树 RSS 分别为 `2.167/0.706/0.914 GiB`，最低可用内存均超过 `95 GiB`，失败不是内存耗尽。下一步先提交 D43 实现，再完成 OpenBayes 同哈希回归；两者通过后才执行唯一一次离线证书恢复，Hybrid、技术排序和全年上界修复继续阻断；
