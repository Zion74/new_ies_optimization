...
- **D46 事后只读诊断与 D48 结果前路线**：诊断 bundle `c74a694...` 未调用求解器；BESS 的 `55,425` 条违约主要来自 CHP 编码/启停轨迹，TES/Hybrid 的 `48,801/48,791` 条违约在幅值上由 HT 接收/送出模式主导，说明逐变量取整不能形成一致离散轨迹。D48 第 1–11 节已在任何实现和正式数值前冻结：撤销 D46 最大容量固定、恢复原容量边界，以完整二元清单的等权 Hamming 距离搜索首个 incumbent；成功后固定全部二元、恢复原成本目标并自由优化连续容量。D48 尚未实现，Gate A 和正式批次均未启动；
- **服务器与当前准入**：OpenBayes 60 核 / 约 100 GB 内存，D46 正式批次结束后活动残留进程为 0；D43、D45、D47、D46 均不得原样重跑。BESS、TES、Hybrid 现均具有至少一个严格全年下界，但没有对应原 MILP 可行上界、容量或项目 TAC。下一步只允许实现并验证已冻结的 D48；正式 E2–E4、gap 收缩、项目 TAC 和技术赢家继续阻断；
