...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；D21–D23 给出风险预算、逐时 PCC 和替代调度包络，D26 建立缩放/严格容差/已知证人证书，D27 用固定支持方向与等价正负分解修正 24 h 最大端并收紧 336 h 全局上界，D28 的两个预注册单步种子均未改善该下界，D29 再以外送耦合有效不等式将 336 h 上界收紧至 `845,052.030831 MWh/a`；D24 将 16 账户统一出表，D25 将四个项目运行账户转成 51 项接收字段和隐私模板。当前仍为 `0/16` 严格账户、`0/4` 项目账户可进入正式复核；完整 TAC 与容量规划未闭合 |
...
| 第 4 章 BESS—TES 适用边界 | 应由哪一种储能提供灵活性 | E1-E6，含四架构、公平前沿、双边界图和 8784 h 验证 | 价值分解、ε 前沿、两张 regime map、全年验证 | `tes_bess_boundary/` 已实现 fixed-capacity、寿命经济、BESS 正式子账本、TES 成本门禁、同 PCC 服务、D21–D23 风险/暴露包络、D26 严格缩放证书、D27 方向/符号重构、D28 多起点负筛查、D29 外送耦合上界紧化、D24 的 16 账户路线和 D25 的项目数据接收门。24 h 严格包络已精确闭合；336 h 最大上界经 D27/D29 累计较 D26 收紧 `37.9619%` 至 `845,052.030831 MWh/a`，但外界仍宽。`0/16` 正式账户、`0/4` 项目接收账户、336 h 闭合、代表周和容量规划完成前，E1-E6 不启动 |
