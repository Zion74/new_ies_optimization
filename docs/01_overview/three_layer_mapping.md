...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；D21–D23 给出风险预算、逐时 PCC 和替代调度包络，D26–D30 建立严格证书并把 336 h 上界收紧至 `777,141.368858 MWh/a`，D31/D32 为负筛查。D24/D25 当前仍为 `0/16` 严格账户、`0/4` 项目账户可复核。D33–D35 建立公开成本、完整内生容量和材料性门；D36 冻结六周代表集、权重及年尾段；D37 已关闭七块循环边界。原 D38 高热状态物理失败；R1 baseline 又发生代表期到真实全年的可行性反转。D40 Gate A 已证明四架构真实全年模型可在线性和资源门内构造，但正式 BESS 单体求解墙钟超限且无可审计结果，全年单体路线未通过。D41 Gate A 已证明三个全年模型的 R0/R1 二元域变换和完整固定无遗漏，但尚未求解合法下界或可行上界；完整 TAC 和严格全年分解证据仍未闭合 |
...
| 第 4 章 BESS—TES 适用边界 | 应由哪一种储能提供灵活性 | E1-E6，含四架构、公平前沿、双边界图和 8784 h 验证 | 价值分解、ε 前沿、两张 regime map、全年验证 | D33–D37 已完成公开成本/容量/材料性/代表周/分块边界。原 D38 高热状态物理失败；R1 baseline 发生代表期—全年反转；D39 虽修复 10% 服务分类，弃电率误差仍达 `5.1762` 个百分点，Gate B 失败并终止 Gate C/D。D40 已通过四架构真实 8784 h 构造与资源 Gate A，代表期不再作为正式容量规划主证据；正式 BESS 在 `4527.395 s` 后仍无结果并被记为 `monolithic_not_viable`，故全年单体路线未通过。D41 结果前合同要求松弛 dual 给合法下界、原始 8784 h 修复给可行上界，并保留 `0.1%/0.5%` 判定门；Gate A 二元覆盖与包含关系审计已通过，Gate B–D 未执行。公开组合与 `0/16`、`0/4` 证据门仍禁止杨凌项目赢家主张 |
