...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；D21–D23 给出风险预算、逐时 PCC 和替代调度包络，D26–D30 建立严格证书，D31/D32 为负筛查。D24/D25 仍为 `0/16` 严格账户、`0/4` 项目账户。D44 已恢复 TES R0/R1 下界，BESS 下界保持；D45 Hybrid R0 唯一正式运行无证书且不得重跑。D47 唯一正式 IPX 56/56 加权持久块完成，恢复 Hybrid R0 严格下界 `232,011,577.8359315691 CNY`，并合法覆盖 Hybrid R1/原 MILP。三架构现均有严格全年下界，但都不是项目 TAC、容量或可行上界，不能单独排序；完整 TAC 未闭合，D46 只获得另立合同的权限 |
...
| 第 4 章 BESS—TES 适用边界 | 应由哪一种储能提供灵活性 | E1-E6，含四架构、公平前沿、双边界图和 8784 h 验证 | 价值分解、ε 前沿、两张 regime map、全年验证 | D33–D37 已完成公开成本/容量/材料性/代表周/分块边界；D38/R1/D39、D40–D43 的失败均保留。D44 关闭 TES 下界缺口；D45 的两条 24 块证书均在冻结墙钟前未完成且不得重跑。D47 已以唯一正式 IPX 56/56 块恢复 Hybrid 下界，三架构严格下界闭合。下一步只允许冻结独立 D46 可行上界/修复合同；容量、gap、技术排序和 E2–E4 批量扫描继续禁止 |
