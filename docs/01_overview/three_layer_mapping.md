...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；D21–D23 给出风险预算、逐时 PCC 和替代调度包络，D26–D30 建立严格证书，D31/D32 为负筛查。D24/D25 仍为 `0/16` 严格账户、`0/4` 项目账户。D44 已恢复 TES R0/R1 下界，BESS 下界保持；D47 恢复 Hybrid R0 下界并覆盖 R1/原 MILP。三架构现均有严格全年下界，但不能用来源不同的下界排序。D46 唯一正式批次三架构均为 `no_candidate_incumbent`。D46 只读诊断已定位离散轨迹一致性问题；D48 结果前合同已冻结等权 Hamming 可行性搜索与固定完整二元后的原成本修复，但尚未实现或运行 |
...
| 第 4 章 BESS—TES 适用边界 | 应由哪一种储能提供灵活性 | E1-E6，含四架构、公平前沿、双边界图和 8784 h 验证 | 价值分解、ε 前沿、两张 regime map、全年验证 | D33–D37 已完成公开成本/容量/材料性/代表周/分块边界；D38/R1/D39、D40–D43 的失败均保留。D44 与 D47 已使三架构严格下界门闭合。D46 上界恢复数为 0；D48 仅获准按冻结合同搜索原容量边界下的一致二元轨迹并修复原成本上界。D48 Gate A/正式批次通过前，正式可行容量、gap、技术排序和 E2–E4 扫描仍阻断 |
