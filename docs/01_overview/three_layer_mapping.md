...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；E0-D-21 将缺证账户转成来源无关的风险预算；E0-D-22 导出逐时 PCC 并证明固定平价抵消、量化价差暴露。TES 正式来源、实际非燃料账户、替代最优轨迹包络、完整 TAC 与容量规划仍未闭合 |
...
| 第 4 章 BESS—TES 适用边界 | 应由哪一种储能提供灵活性 | E1-E6，含四架构、公平前沿、双边界图和 8784 h 验证 | 价值分解、ε 前沿、两张 regime map、全年验证 | `tes_bess_boundary/` 已实现 fixed-capacity、寿命经济、BESS 正式子账本、TES 成本门禁、同 PCC 服务诊断、非燃料证据门、D21 影子成本区间传播和 D22 逐时价差暴露；当前所选轨迹的重新分配电量为 26.010–31.228 GWh/a，结算单账户反事实临界价差为约 481–523 CNY/MWh，但这不是项目价格、完整 TAC 或技术赢家。12 个 TES 账户、替代最优包络、代表周和容量规划闭合前，E1-E6 不启动 |
