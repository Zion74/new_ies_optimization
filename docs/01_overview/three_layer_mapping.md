...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-13 已将 Rahman 关联证据包登记为唯一 BESS 正式来源候选并完成主要非电芯映射；TES 正式来源仍为零，BESS 三个模型接缝和完整 TAC 未闭合。本地 `263 passed`，OpenBayes 最近 `258 passed` |
