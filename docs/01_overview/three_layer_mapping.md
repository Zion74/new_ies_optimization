...
| 第 2 章 系统、数据与统一模型 | 杨凌对象和物理/经济口径是否可信 | 数据审计、CHP 可行域、BESS/TES 单元验证、正式输入桥接、HiGHS 验证、成本证据闭环 | 系统边界、数据覆盖、可行域、模型误差、证据资格矩阵 | E0-D-14–D-20 已闭合 BESS fixed-capacity 生命周期账本、TES 正式成本门、同 PCC 燃料空间和四类非燃料成本证据审计；D21–D23 给出风险预算、逐时 PCC 和替代调度包络，D26 进一步建立缩放/严格容差/已知证人的数值证书；D24 将 16 账户统一出表，D25 将四个项目运行账户转成 51 项接收字段和隐私模板。当前仍为 `0/16` 严格账户、`0/4` 项目账户可进入正式复核；完整 TAC 与容量规划未闭合 |
